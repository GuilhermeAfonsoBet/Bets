...
**Data da execução:** 18/02/2026 21:55 UTC  
...
- **Amostra**: 4026 auditorias (jogos únicos=328, média=12.3 obs/jogo); betslip confiável=1809.
- **Janela efetiva (audited_at)**: 12/02 18:29 → 18/02 21:54 UTC (span≈6.1d; dias com dados=7).
- **Coortes (status=OK, betslip confiável)**: `BS>WS` (diff>=2.0%): **641**; `BS<WS` (diff<=-2.0%): **206**.
- **Coberturas em `hypothesis_details` (OK)**: temporal(back)=1501/1809; lay_temporal=1358/1809; finance=1269/1809.
- **Cobertura de placar (ROI)**: jogos com placar=279/328 (status finished=279).
- **Cobertura de closing_odd (AH)**: jogos com closing=197/328 (60.1%). CLV pre‑match depende disso.
...
- **CLV pre-match (Betslip, API)**: média robusta por jogo +2.125% (IC90 [+1.668%, +2.568%]), com N=938 eventos (jogos=158).
...
| Indicador | Valor |
|---|---:|
| Auditorias H3B `UP` (match + kickoff passado) | 4026 |
| Betslip bruto | 2866 |
| Betslip confiável (diff -10% a +10%) | 1809 |
| Descartados no filtro de qualidade | 1057 |
| Jogos únicos (geral) | 328 |
| Média de observações por jogo | 12.3 |
| Jogos únicos com betslip confiável | 297 |
| Distribuição por market_type | AH=4026 |
| Jogos únicos (AH) no recorte | 328 |
| Jogos únicos (AH) com closing_odd disponível | 197 |
| Cobertura closing_odd (AH) | 60.1% |

---
## 2) Base comparativa: API vs DOM
| Métrica | API (v4.0-api) | DOM (v1.0) |
|---|---:|---:|
| Total de observações | 4026 | 0 |
| Com betslip confiável | 1809 | 0 |
| Com CLV pre-match (betslip) | 938 | 0 |
| Com ROI (betslip) | 1566 | 0 |
| Tempo total observado (detecção→betslip, wall/total) | 12291 ms | — ms |
| Tempo instrumentado (detecção→clique→betslip) | 8817 ms | — ms |
...
|---|---|---:|---:|---:|---:|
| API (2-4s) | lag_det→click | 5560 | 796 | 7082 | 4024 |
| API (2-4s) | lag_click→betslip | 2627 | 2172 | 4510 | 3406 |
| API (2-4s) | lag_e2e (soma) | 8817 | 3462 | 11102 | 3404 |
| API (2-4s) | audit_total (duração) | 12287 | 4510 | 31068 | 4026 |
| API (2-4s) | overhead (total - e2e) | 4774 | 204 | 22802 | 3404 |
...
|---|---:|---:|---|
| Observações totais com classificação temporal | 1857 | 2169 | Contagem bruta do corte |
| ROI Betslip | 924 | 642 | Amostra com resultado do jogo |
| ROI WebSocket | 1463 | 1819 | Referência de mercado |
| CLV (apenas pre-match) | 938 | — | CLV vs closing pré-jogo não é interpretável in-match |

---
### 2.2 Performance por regime (pre-match vs in-match)
| Regime | N auditorias | N betslip conf. | N OK | Back edge | Lay edge | Diff médio (OK) |
|---|---:|---:|---:|---:|---:|---:|
| PRE_MATCH | 1857 | 1047 | 1047 | 415 | 69 | +2.233% |
| IN_MATCH | 2169 | 762 | 762 | 226 | 137 | +0.926% |
...
| 20-40s | 194 | 100 | 27136 | 33422 | 29829 | 67 | 20 | +2.40% [+1.45%, +3.41%] | +6.26% [-7.09%, +19.68%] |
| > 40s | 76 | 55 | 187614 | 507883 | 389950 | 34 | 7 | +2.54% [+1.37%, +3.68%] | -12.49% [-32.66%, +7.36%] |
...
| 20-40s | 194 | 67 | 20 | +7.43% [+6.38%, +8.48%] | -1.06% [-3.91%, +2.13%] | +2.78% [-17.64%, +23.11%] | -21.13% [-60.87%, +19.23%] |
| > 40s | 76 | 34 | 7 | +5.38% [+3.68%, +7.04%] | -4.80% [-7.29%, -2.29%] | -27.27% [-56.61%, +2.88%] | -49.64% [-100.00%, +0.00%] |
| Desconhecido | 0 | 0 | 0 | — | — | — | — |
...
| Métrica | API | DOM |
|---|---:|---:|
| CLV Bruto BS Pre-Match | +2.336% (sig. positivo, N=938, jogos=158) | — (N/A, N=0, jogos=0) |
| CLV Adicional BS Pre-Match | +1.763% (sig. positivo, N=938, jogos=158) | — (N/A, N=0, jogos=0) |
| Taxa de CLV > 0 (bruto) | 61.2% | —% |
| Taxa de CLV > 0 (adicional) | 61.9% | —% |

Notas de robustez (IC 90% por jogo):  
- API CLV bruto (cluster): média +2.125%; IC90 [+1.668%, +2.568%]  
...
| Métrica | API | DOM |
|---|---:|---:|
| ROI Betslip | +1.373% (NS, N=1564) | — (N/A, N=0) |
| ROI WebSocket | -0.691% (NS, N=3267) | — (N/A, N=0) |
| Win rate ROI Betslip | 50.7% | —% |
| Win rate ROI WS | 50.1% | —% |

Notas de robustez (IC 90% por jogo):  
- API ROI betslip (cluster): média +1.380%; IC90 [-3.867%, +6.455%]  
- API ROI WS (cluster): média -2.977%; IC90 [-6.001%, +0.028%]  

---
## 5) Diferença de preço BS vs WS
| Métrica | API | DOM |
|---|---:|---:|
| Diff BS vs WS (média) | +1.682% (sig. positivo, N=1809) | — (N/A, N=0) |
| BS > WS | 47.7% (863/1809) | —% (0/0) |
| BS > WS +2% | 35.4% (641/1809) | —% (0/0) |
...
|---|---:|---:|---|---:|---:|---:|---|
| BS < WS (-10% a -2%) | 206 | -3.920% | [-4.884%, -3.048%] | 59 | 46 | +0.355% | [-17.043%, +7.079%] |
| BS ~ WS (-2% a +2%) | 962 | -0.132% | [-0.539%, +0.188%] | 498 | 143 | +1.000% | [-9.343%, +3.589%] |
| BS > WS (+2% a +10%) | 641 | +6.532% | [+6.382%, +7.155%] | 381 | 119 | +2.256% | [-2.288%, +14.998%] |

---
### 6.2 Combinação por faixa de linha AH
| Faixa AH | CLV BS PM (média) | IC90 (cluster) | ROI BS (todos) | IC90 (cluster) | Diff BS vs WS (média) |
|---|---:|---|---:|---|---:|
| AH 0-1 (líquida) | +2.573% | [+2.148%, +3.479%] | +1.369% | [+0.481%, +15.601%] | +1.889% |
...
| 20-30s | +2.249% | [+1.415%, +3.388%] | 122 | 72 | +4.506% | [-7.560%, +20.494%] | +1.815% |
| > 30s | +2.131% | [+1.404%, +3.539%] | 71 | 48 | -10.848% | [-28.826%, +5.369%] | +1.930% |
...
| stake_cap | 0.00 |
| Cobertura finance (OK, betslip conf.) | 1269/1809 |
...
| N com ROI realizado | 556 |
| P&L realizado total (estimado) | -34461.22 |
| ROI realizado (ponderado por stake) | -17.88% |
| ROI realizado (robusto por jogo, mean; IC90) | +6.25% [-2.29%, +15.00%] |
| ROI ponderado por stake (robusto por jogo, mean; IC90) | +6.15% [-4.97%, +17.77%] |
...
| N com ROI realizado | 124 |
| P&L realizado total (estimado) | -7605.38 |
| ROI realizado (ponderado por liability) | -27.05% |
| ROI realizado (ponderado por stake) | -23.75% |
| ROI/liability (robusto por jogo, mean; IC90) | +18.66% [+2.71%, +34.68%] |
| ROI/liability ponderado (robusto por jogo, mean; IC90) | +16.87% [+0.76%, +33.46%] |
...
|---|---:|---:|---:|---:|
| Back | 7.0 | 862399.41 | -147690.95 | -154156.53 |
| Lay (stake) | 7.0 | 147428.62 | -32594.50 | -35009.49 |
| Total (Back+Lay) | 7.0 | 1009828.03 | -180285.45 | -189166.02 |
...
|---|---:|---:|---:|---:|
| Back (stake) | 4394.24 | 3831.80 | -3361.02% | -3508.15% |
| Lay (liability) | 2045.09 | 1568.72 | -1593.80% | -1711.88% |
| Total (soma) | 6439.32 | 5400.52 | -2799.76% | -2937.67% |
...
| ROI/banca 30d (direto, banca efetiva) | -178.89% |
| ROI/banca 30d (ROI×turnover, banca efetiva) | -187.70% |
**Diagnóstico de cobertura (placar/ROI)**

| Bloco | Turnover total | Turnover com ROI | Cobertura turnover |
|---|---:|---:|---:|
| Back | 201226.53 | 192786.76 | 95.81% |
| Lay | 34400.01 | 32027.07 | 93.10% |

Notas (Lay): exposição 30d por liability (não é turnover) = 129865.51; ROI realizado por liability (ponderado) = -27.05%.
...
|---|---:|---:|---:|---:|---:|---:|---:|
| PRE_MATCH | 1047 | 4.5 | 5.2 | 76.2% | 17.0% | 12.2 | 7.1 |
...
|---|---:|---:|---:|---:|---:|
| t+0s | 1809 | +1.68% | 2.017 | +2.34% | 2.13 |
...
|---|---:|---:|---:|---:|---:|
| SEM_REVERSAO | 1388 | 781 | +1.77% [+1.34%, +2.19%] | +2.04% [+1.61%, +2.46%] | +2.03% [+1.60%, +2.45%] |
...
|---|---:|---:|---:|---:|---:|
| SEM_REVERSAO | 1388 | 1206 | +1.49% [-4.69%, +7.85%] | +1.67% [-4.51%, +8.03%] | +1.65% [-4.53%, +8.00%] |
...
|---|---:|---:|---:|---:|
| SEM_REVERSAO | 781 | 1.983 [+1.972, +1.994] | 1.991 [+1.979, +2.002] | 1.964 [+1.955, +1.972] |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| PRE_MATCH | 927 | 4.3 | 5.2 | 69.7% | 21.5% | 12.6 | 7.0 |
...
|---|---:|---:|---:|---:|---:|
| t+0s | 1533 | +0.67% | 2.000 | +0.05% | 10.79 |
...
|---|---:|---:|---:|---:|---:|
| SEM_REVERSAO | 1071 | 647 | -0.41% [-1.03%, +0.22%] | -0.07% [-0.69%, +0.54%] | -0.10% [-0.71%, +0.52%] |
...
|---|---:|---:|---:|---:|---:|
| SEM_REVERSAO | 1071 | 934 | +5.65% [-3.23%, +14.82%] | +6.36% [-2.63%, +15.74%] | +6.32% [-2.66%, +15.68%] |
...
|---|---|---|---:|---:|---:|---:|---:|---|
| Back | Pre | Any | 1047 | 189 | +2.12% [+1.67%, +2.57%] | -4.29% [-11.38%, +2.77%] | -6.43% | não (CLV p90>0 AND ROI>0) |
| Back | In | Any | 762 | 217 | — | +5.04% [-2.01%, +12.01%] | +2.67% | sim (ROI p30>0) |
| Lay | Pre | Yes | 199 | 105 | -0.15% [-1.07%, +0.76%] | +3.51% [-9.69%, +17.11%] | -0.66% | não (CLV_CONV p90>0 AND ROI p30>0) |
| Lay | Pre | No | 728 | 172 | +0.41% [-0.22%, +1.03%] | +0.73% [-7.71%, +8.93%] | -1.98% | não (CLV_CONV p90>0 AND ROI p30>0) |
...
- **Lay (liability)**: corr(exposição, ROI)=0.078; corr(exposição, CLV)=0.061 (onde CLV existe).
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Back | FLAT | 556 | 556.00 | 19.02 | 3.42% | 1.00 | 1.00 | 7.19 | 12.77 |
| Lay | FLAT | 175 | 214.91 | 1.66 | 0.77% | 1.00 | 1.00 | 23.02 | 44.64 |
| Back | PROXY | 556 | 192786.76 | -34461.22 | -17.88% | 4396.70 | 4212.57 | 173385.00 | 247604.67 |
| Lay | PROXY | 124 | 32027.07 | -7605.38 | -23.75% | 3858.16 | 2142.82 | 45930.30 | 61559.80 |
| Back | KELLY_0.10 | 318 | 8251.67 | 380.55 | 4.61% | 56.97 | 51.27 | 586.70 | 1102.81 |
| Lay | KELLY_0.10 | 44 | 585.12 | 105.57 | 18.04% | 20.45 | 20.45 | 22.25 | 37.14 |
| Back | KELLY_0.25 | 318 | 16861.96 | 509.28 | 3.02% | 87.88 | 87.88 | 1291.31 | 2430.82 |
| Lay | KELLY_0.25 | 44 | 840.83 | 171.38 | 20.38% | 20.45 | 20.45 | 22.24 | 36.15 |
| Back | KELLY_0.50 | 318 | 20567.86 | 443.31 | 2.16% | 87.88 | 87.88 | 1352.92 | 2617.16 |
| Lay | KELLY_0.50 | 44 | 926.44 | 159.43 | 17.21% | 20.45 | 20.45 | 12.40 | 20.12 |
| Back | KELLY_1.00 | 318 | 21244.08 | 434.13 | 2.04% | 87.88 | 87.88 | 1386.87 | 2684.79 |
| Lay | KELLY_1.00 | 44 | 941.57 | 168.48 | 17.89% | 20.45 | 20.45 | 0.00 | 0.00 |
...
|---|---|---|---|---:|---:|---:|---:|---:|---:|
| Back | Pre | Yes | FLAT | 91 | 91.00 | -0.33 | -0.36% | 1.00 | 52.84 |
| Back | Pre | Yes | PROXY | 91 | 30945.32 | -8836.63 | -28.56% | 4734.71 | 84745.12 |
| Back | Pre | Yes | KELLY_0.10 | 80 | 2251.97 | -93.66 | -4.16% | 47.96 | 1496.90 |
| Back | Pre | Yes | KELLY_0.25 | 80 | 4798.30 | -78.98 | -1.65% | 87.88 | 2121.13 |
| Back | Pre | Yes | KELLY_0.50 | 80 | 5766.77 | 17.81 | 0.31% | 87.88 | 1778.86 |
| Back | Pre | Yes | KELLY_1.00 | 80 | 5893.45 | -14.73 | -0.25% | 87.88 | 2130.68 |
| Back | Pre | No | FLAT | 278 | 278.00 | 10.61 | 3.82% | 1.00 | 23.02 |
| Back | Pre | No | PROXY | 278 | 122098.36 | -16089.93 | -13.18% | 4433.26 | 151004.92 |
| Back | Pre | No | KELLY_0.10 | 238 | 5999.70 | 474.21 | 7.90% | 56.97 | 764.03 |
| Back | Pre | No | KELLY_0.25 | 238 | 12063.66 | 588.26 | 4.88% | 87.88 | 1691.62 |
| Back | Pre | No | KELLY_0.50 | 238 | 14801.09 | 425.50 | 2.87% | 87.88 | 1906.32 |
| Back | Pre | No | KELLY_1.00 | 238 | 15350.63 | 448.86 | 2.92% | 87.88 | 1757.93 |
| Back | In | Yes | FLAT | 58 | 58.00 | -4.52 | -7.80% | 1.00 | 50.23 |
| Back | In | Yes | PROXY | 58 | 9973.72 | -3099.84 | -31.08% | 1457.62 | 34601.08 |
| Back | In | No | FLAT | 129 | 129.00 | 13.26 | 10.28% | 1.00 | 0.00 |
| Back | In | No | PROXY | 129 | 29769.35 | -6434.83 | -21.62% | 2077.23 | 66845.21 |
| Lay | Pre | Yes | FLAT | 13 | 14.38 | 3.45 | 24.02% | 1.00 | 6.82 |
| Lay | Pre | Yes | PROXY | 13 | 6138.31 | -3626.31 | -59.08% | 3925.76 | 40415.44 |
| Lay | Pre | Yes | KELLY_0.10 | 9 | 96.94 | 16.34 | 16.85% | 20.09 | 115.32 |
| Lay | Pre | Yes | KELLY_0.25 | 9 | 149.48 | 35.07 | 23.46% | 20.45 | 96.10 |
| Lay | Pre | Yes | KELLY_0.50 | 9 | 179.82 | 41.42 | 23.04% | 20.45 | 98.47 |
| Lay | Pre | Yes | KELLY_1.00 | 9 | 182.11 | 43.71 | 24.00% | 20.45 | 98.52 |
| Lay | Pre | No | FLAT | 32 | 36.66 | 2.74 | 7.49% | 1.00 | 3.13 |
| Lay | Pre | No | PROXY | 32 | 10392.85 | -3622.01 | -34.85% | 3404.75 | 36832.93 |
| Lay | Pre | No | KELLY_0.10 | 24 | 358.17 | 26.55 | 7.41% | 20.45 | 46.41 |
| Lay | Pre | No | KELLY_0.25 | 24 | 498.98 | 55.37 | 11.10% | 20.45 | 28.20 |
| Lay | Pre | No | KELLY_0.50 | 24 | 538.91 | 38.89 | 7.22% | 20.45 | 75.73 |
| Lay | Pre | No | KELLY_1.00 | 24 | 538.91 | 38.89 | 7.22% | 20.45 | 75.30 |
| Lay | In | Yes | FLAT | 26 | 31.49 | 3.39 | 10.77% | 1.00 | 5.04 |
| Lay | In | Yes | PROXY | 26 | 5812.58 | 1881.87 | 32.38% | 1713.54 | 3036.37 |
| Lay | In | No | FLAT | 49 | 76.33 | -0.08 | -0.10% | 1.00 | 22.84 |
| Lay | In | No | PROXY | 49 | 9137.55 | -2645.04 | -28.95% | 1043.16 | 22287.97 |
...
| Back | Pre | No | 869 | 176 | +1.77% [+1.34%, +2.19%] | -2.24% [-9.56%, +5.50%] | -4.73% | pre: Kelly OK |
...
| Lay | Pre | No | 728 | 172 | +0.10% [-0.52%, +0.71%] | +0.73% [-7.71%, +8.93%] | -1.98% | pre: Kelly OK |
...
| Com ROI disponível (precisa de placar) | 206 |
...
| Tipo | Dias |
|---|---:|
| Dias com dados no recorte (OK) | 5 |
...
|---|---|---:|---:|---:|---:|---:|
| 2026-02-12→2026-02-14 | 2026-02-15→2026-02-15 | 1 | 40 | +11.09% [-10.85%, +33.49%] | 737.80 | 162.78 |
| 2026-02-12→2026-02-15 | 2026-02-16→2026-02-16 | 3 | 36 | +7.68% [-25.77%, +47.32%] | 865.51 | -7.56 |
...
**Regra de elegibilidade (todas as combinações):** exige `N_ROI >= wf_min_matches` (aqui: 30).
...
|---|---|---:|---:|---:|---|
| Back_Pre_Any | SIM | 99 / 91 / 88 | q10=6.19 | CI90_lb=6.05 | +6.50% | BackPre: clv_q10>0=True, roi_mean>0=True |
| Back_In_Any | NÃO | 81 / — / 69 | — | -2.72% | BackIn: roi_q30>0 AND N_ROI>=min (N=69/30) |
| Lay_Pre_Yes | NÃO | 8 / 6 / 8 | q10=-8.32 | CI90_lb=-9.09 | -0.87% | LayPre: clv_conv_q10>0=False, roi_q30>0=False |
| Lay_Pre_No | NÃO | 29 / 23 / 23 | q10=-6.94 | CI90_lb=-7.45 | +8.97% | LayPre: clv_conv_q10>0=False, roi_q30>0=False |
| Lay_In_Yes | NÃO | 11 / — / 7 | — | +2.44% | In: roi_q30>0 AND N_ROI>=min (N=7/30) |
| Lay_In_No | NÃO | 31 / — / 29 | — | +1.03% | In: roi_q30>0 AND N_ROI>=min (N=29/30) |

**Train 2026-02-12→2026-02-15 → Test 2026-02-16→2026-02-16**

| Combinação | Ativa? | Jogos treino (tot/CLV/ROI) | CLV q10/q90 ou CI | ROI q30 | Motivo |
|---|---|---:|---:|---:|---|
| Back_Pre_Any | SIM | 121 / 108 / 108 | q10=6.42 | CI90_lb=6.32 | +8.40% | BackPre: clv_q10>0=True, roi_mean>0=True |
| Back_In_Any | SIM | 123 / — / 107 | — | +1.11% | BackIn: roi_q30>0 AND N_ROI>=min (N=107/30) |
| Lay_Pre_Yes | NÃO | 12 / 9 / 12 | q10=-6.21 | CI90_lb=-6.83 | +6.65% | LayPre: clv_conv_q10>0=False, roi_q30>0=False |
| Lay_Pre_No | NÃO | 33 / 27 / 27 | q10=-6.53 | CI90_lb=-6.92 | +6.79% | LayPre: clv_conv_q10>0=False, roi_q30>0=False |
| Lay_In_Yes | NÃO | 25 / — / 19 | — | +6.49% | In: roi_q30>0 AND N_ROI>=min (N=19/30) |
| Lay_In_No | SIM | 41 / — / 38 | — | +2.04% | In: roi_q30>0 AND N_ROI>=min (N=38/30) |
...
| Dias OOS usados | 2 |
| Turnover 30d (proj.) | 24049.66 |
| Turnover 30d (Pre/In) | 93885.10 / 363.02 |
| Lucro 30d (obs.) | 2109.83 |
| Lucro 30d (obs.) Pre/In | 4004.65 / 78.42 |
| Lucro 30d (exp.) | 2328.26 |
| Lucro 30d (exp.) Pre/In | 4534.62 / 110.71 |
| Banca risco p99 (Back+Lay) | 864.08 |
| Banca liquidez p99 (+buf) | 1374.55 |
| Banca recomendada (max) | 1374.55 |
| ROI/banca 30d (obs.) | 153.49% |
| ROI/banca 30d (exp.) | 169.38% |
| DD 30d p95 (obs.) | 43.20 |
| DD 30d p95 (exp.) | 52.93 |
...
|---|---:|---:|---:|---:|---:|
| BASELINE (sem budget) | 24049.66 | 2328.26 | 1374.55 | 169.38% | 52.93 |
| BUDGET_0.50%/0.25% cap25% | 3340.75 | 346.90 | 131.18 | 264.45% | 0.00 |
| BUDGET_1.00%/0.50% cap33% | 7924.71 | 801.77 | 291.66 | 274.90% | 0.00 |
| BUDGET_2.00%/1.00% cap50% | 19939.15 | 1429.36 | 683.45 | 209.14% | 336.43 |
| BUDGET_3.00%/1.50% cap33% | 22640.13 | 2189.03 | 814.03 | 268.91% | 51.89 |
| BUDGET_4.00%/2.00% cap33% | 29734.37 | 2970.33 | 1064.30 | 279.09% | 22.60 |
| BUDGET_3.00%/1.50% cap50% | 29081.37 | 2346.80 | 987.54 | 237.64% | 345.02 |
| BUDGET_4.00%/2.00% cap50% | 37251.07 | 3541.23 | 1258.89 | 281.30% | 192.69 |
...
| Lay_In_No | 1 | 2 | 5 | 1.04 | budget reduz concentração por jogo |
...
| Indicador | Valor |
|---|---:|
| Jogos únicos no recorte | 328 |
| Jogos com placar disponível (home_score/away_score não nulos) | 279 |
| Jogos com status='finished' no banco | 279 |

### 10.1 Distribuição por data de kickoff (explica janela da API)
- Kickoff (UTC) no recorte: **2026-02-12 15:00 UTC** até **2026-02-18 20:00 UTC**.
