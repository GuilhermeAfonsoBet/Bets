...
- **PnL semanal médio (bootstrap IC95%)**: **USD 218.2** (IC95% -110.1..558.2)
- **Desvio padrão semanal**: USD 709.7
- **P(semana < 0)**: 43.8%

### Performance OOS (cap2) — excluindo semanas sem trades (stake=0)
- **PnL semanal médio (bootstrap IC95%)**: **USD 232.8** (IC95% -114.9..600.7)
- **Desvio padrão semanal**: USD 732.1
- **P(semana < 0)**: 46.7%
- **ROI on stake agregado (ponderado)**: 0.0455

### Risco no OOS (teste) — portfólio agregado
- p80(soma stakes/dia) = USD 2019 (limite=USD 1610)
- VaR10%(PnL diário) = USD -277.2 (limite >= USD -575)
