...
- **PnL semanal médio (bootstrap IC95%)**: **USD 246.5** (IC95% -1.5..542.7)
- **Desvio padrão semanal**: USD 572.3
- **P(semana < 0)**: 37.5%

### Performance OOS (cap2) — excluindo semanas sem trades (stake=0)
- **PnL semanal médio (bootstrap IC95%)**: **USD 262.9** (IC95% -1.9..570.7)
- **Desvio padrão semanal**: USD 588.4
- **P(semana < 0)**: 40.0%
- **ROI on stake agregado (ponderado)**: 0.0572

### Risco no OOS (teste) — portfólio agregado
- p80(soma stakes/dia) = USD 1958 (limite=USD 1610)
- VaR10%(PnL diário) = USD -171.4 (limite >= USD -575)
- P(PnL diário <= -25% banca) = 0.0% (limite <= 10%)

### Ajuste de stake global (α)
- α médio=0.950; p10=0.798; p50=1.000; p90=1.000; P(α<1)=31.2%
...
- **FT|terça-feira**: mean_week=104.0 (IC95% 6.9..246.2), P(semana>0)=43.8%
- **FT|segunda-feira**: mean_week=64.0 (IC95% -6.4..137.3), P(semana>0)=50.0%
- **FT|sábado**: mean_week=56.6 (IC95% -4.6..124.5), P(semana>0)=37.5%
- **FH|sábado**: mean_week=31.3 (IC95% -10.0..80.3), P(semana>0)=43.8%
- **FT|quarta-feira**: mean_week=27.9 (IC95% -80.1..138.1), P(semana>0)=37.5%
- **FH|terça-feira**: mean_week=9.6 (IC95% -34.6..52.5), P(semana>0)=18.8%
- **FH|quinta-feira**: mean_week=5.5 (IC95% -60.0..80.4), P(semana>0)=31.2%
