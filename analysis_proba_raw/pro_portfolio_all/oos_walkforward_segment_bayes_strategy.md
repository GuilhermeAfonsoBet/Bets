...
- **PnL semanal médio (bootstrap IC95%)**: **USD 172.6** (IC95% -120.5..503.2)
- **Desvio padrão semanal**: USD 661.4
- **P(semana < 0)**: 50.0%

### Performance OOS (cap2) — excluindo semanas sem trades (stake=0)
- **PnL semanal médio (bootstrap IC95%)**: **USD 172.6** (IC95% -121.7..506.4)
- **Desvio padrão semanal**: USD 661.4
- **P(semana < 0)**: 50.0%
- **ROI on stake agregado (ponderado)**: 0.0412

### Risco no OOS (teste) — portfólio agregado
- p80(soma stakes/dia) = USD 1599 (limite=USD 1610)
