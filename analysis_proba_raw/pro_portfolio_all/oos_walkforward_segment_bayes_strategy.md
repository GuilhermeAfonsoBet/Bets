...
- **PnL semanal médio (bootstrap IC95%)**: **USD -11.7** (IC95% -194.5..172.1)
- **Desvio padrão semanal**: USD 381.5
- **P(semana < 0)**: 50.0%

### Performance OOS (cap2) — excluindo semanas sem trades (stake=0)
- **PnL semanal médio (bootstrap IC95%)**: **USD -11.7** (IC95% -188.2..171.5)
- **Desvio padrão semanal**: USD 381.5
- **P(semana < 0)**: 50.0%
- **ROI on stake agregado (ponderado)**: -0.0073

### Risco no OOS (teste) — portfólio agregado
- p80(soma stakes/dia) = USD 772 (limite=USD 1610)
- VaR10%(PnL diário) = USD -336.8 (limite >= USD -575)
...
- **FH|quinta-feira**: mean_week=42.1 (IC95% -29.9..135.8), P(semana>0)=31.2%
...
- **FT|quarta-feira**: mean_week=-32.8 (IC95% -89.9..0.0), P(semana>0)=0.0%
