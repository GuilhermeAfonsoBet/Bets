...
- **PnL semanal médio (bootstrap IC95%)**: **USD -38.9** (IC95% -163.2..85.3)
- **Desvio padrão semanal**: USD 260.1
- **P(semana < 0)**: 43.8%

### Performance OOS (cap2) — excluindo semanas sem trades (stake=0)
- **PnL semanal médio (bootstrap IC95%)**: **USD -51.8** (IC95% -215.8..111.2)
- **Desvio padrão semanal**: USD 302.5
- **P(semana < 0)**: 58.3%
- **ROI on stake agregado (ponderado)**: -0.0294

### Risco no OOS (teste) — portfólio agregado
- p80(soma stakes/dia) = USD 760 (limite=USD 1610)
- VaR10%(PnL diário) = USD -203.1 (limite >= USD -575)
...
- **FT | quarta-feira**: active_rate=68.8%, ok_rate=68.8%, stake_frac_médio=2.50%, cutoff_médio=0.63
...
- **FT|quarta-feira**: mean_week=-26.2 (IC95% -105.3..49.2), P(semana>0)=12.5%
