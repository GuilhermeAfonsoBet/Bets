...
# Fase 2B: telemetria E2E fail-open (nunca bloqueia execução)
try:
    from ops.h3bup_e2e_trace import (
        emit_trace_event as _e2e_emit,
        make_trace_id as _e2e_make_trace_id,
        wall_ts_to_iso as _e2e_wall_ts_to_iso,
        is_enabled as _e2e_is_enabled,
    )
...
class H3bApiAudit:
        ...
        for line_data in lines:
            ...
            for h3b in det.get("h3b_events", []):
                ...
                try:
                    ...
                    _ws_recv_ts = float(getattr(self, "_last_ws_time", 0.0) or 0.0) or _detected_at
                    _trace_id = None
                    try:
                        _trace_id = _e2e_make_trace_id(None)
                    except Exception:
                        _trace_id = None
                    queue.put_nowait({
                        'event_id': event_id,
                        ...
                        'ws_received_ts': _ws_recv_ts,
                        '_e2e_trace_id': _trace_id,
                        'queue_depth_at_enqueue': queue_depth_at_enqueue,
                    })
                    try:
                        if _trace_id:
                            _meta_ws = {"message_bytes": None, "source": "last_ws_before_detect"}
                            _e2e_emit(
                                "H3B_WS_RECEIVED",
                                trace_id=_trace_id,
                                event_id=event_id,
                                ...
                                line=str(h3b.ah_line),
                                service="audit_h3b",
                                event_ts_utc=_e2e_wall_ts_to_iso(_ws_recv_ts),
                                metadata=_meta_ws,
                            )
                            _e2e_emit(
                                "H3B_DETECTED",
                                ...
                                line=str(h3b.ah_line),
                                service="audit_h3b",
                                metadata={"direction": getattr(h3b, "direction_after", None), "ws_odd": getattr(h3b, "odd_at_reversal", None)},
                            )
                            _e2e_emit(
                                "H3B_AUDIT_ENQUEUED",
                                trace_id=_trace_id,
                                ...
                                line=str(h3b.ah_line),
                                service="audit_h3b",
                                metadata={"queue_depth": queue_depth_at_enqueue},
                            )
                    except Exception:
                        pass
    ...
    async def _executor_loop(self, queue: asyncio.Queue, worker_id: int = 1):
        ...
        while self.running:
            ...
            if self.mode in ("ws_gate_lay", "gate_lay", "gate"):
                ...
                try:
                    if isinstance(result, dict) and not result.get("_e2e_trace_id") and h3b.get("_e2e_trace_id"):
                        result["_e2e_trace_id"] = h3b.get("_e2e_trace_id")
                    if isinstance(result, dict) and result.get("ws_received_ts") is None and h3b.get("ws_received_ts") is not None:
                        result["ws_received_ts"] = h3b.get("ws_received_ts")
                except Exception:
                    pass
            elif self.mode in ("ws_only", "ws"):
    ...
    async def _execute_ws_gate_back(self, h3b: dict, *, defer_temporal: bool = True) -> dict:
        ...
        execution_start = time.time()
        queue_wait_ms = int((execution_start - detected_at) * 1000)
        _gate_trace_id = h3b.get("_e2e_trace_id")
        _gate_mono0 = time.monotonic_ns()
        _audit_gate_started_ts = execution_start
        ...
        if not bool(self.gate_back_enforce_rise_filter):
            ...
            try:
                if _gate_trace_id:
                    base['_e2e_trace_id'] = _gate_trace_id
                    if h3b.get('ws_received_ts') is not None:
                        base['ws_received_ts'] = h3b.get('ws_received_ts')
                    _dur = None
                    try:
                        _dur = (time.monotonic_ns() - _gate_mono0) / 1e6
                    except Exception:
                        _dur = None
                    _e2e_emit(
                        "H3B_AUDIT_GATE_DECIDED",
                        trace_id=_gate_trace_id,
                        event_id=h3b.get("event_id"),
                        market_type=h3b.get("market_type"),
                        side=h3b.get("side"),
                        line=h3b.get("line"),
                        status="OK",
                        reason="t0_approve_enforce_rise_filter_off",
                        service="audit_h3b",
                        duration_ms=_dur,
                        metadata={
                            "is_valid_opportunity": True,
                            "ws_odd": ws0,
                            "audit_gate_started_ts": _audit_gate_started_ts,
                            "audit_gate_finished_ts": time.time(),
                        },
                    )
            except Exception:
                pass
    ...
    async def _save_result(self, r: dict):
        if not self.db:
            return None
        _persist_trace = None
        _persist_mono0 = time.monotonic_ns()
        try:
            _persist_trace = r.get("_e2e_trace_id")
            if _persist_trace:
                _e2e_emit(
        ...
        try:
            ...
            try:
                if r.get("_e2e_trace_id"):
                    hypothesis_details["_e2e_trace_id"] = str(r.get("_e2e_trace_id"))
                if r.get("ws_received_ts") is not None:
                    hypothesis_details["ws_received_ts"] = float(r.get("ws_received_ts"))
            except Exception:
                pass
            ...
            try:
                async with self.db.async_session() as session:
                    ...
                    try:
                        if _persist_trace:
                            _dur = (time.monotonic_ns() - _persist_mono0) / 1e6
                            _e2e_emit(
                                ...
                                service="audit_h3b",
                                duration_ms=_dur,
                                metadata={"is_valid_opportunity": bool(is_valid)},
                            )
                    except Exception:
                        pass
                    return _rid
