...
class ExecutorWorker:
    ...
    async def execute_dryrun(self, req: ExecutionRequest, received_ts: float) -> ExecutionResult:
        ...
        _tid = None
        _dry_mono0 = time.monotonic_ns()
        try:
            _tid = _e2e_from_meta(getattr(req, "meta", None))
            if _tid:
                _e2e_emit(
                    "H3B_DRYRUN_STARTED",
                    trace_id=_tid,
                    audit_id=getattr(req, "audit_id", None),
                    execution_id=str(getattr(req, "execution_id", "") or ""),
                    event_id=getattr(req, "event_id", None),
                    market_type=str(getattr(getattr(req, "market_type", None), "value", getattr(req, "market_type", None))),
                    side=getattr(req, "side", None),
                    line=getattr(req, "line", None),
                    policy_version=str(getattr(getattr(req, "policy", None), "policy_version", None) or ""),
                    service="executor",
                    metadata={"dryrun_started_ts": t0},
                )
        except Exception:
            pass
    ...
    async def _execute_unlocked(self, req: ExecutionRequest, received_ts: float) -> ExecutionResult:
            ...
            try:
                ...
                if _tid:
                    _e2e_emit(
                        ...
                        line=getattr(req, "line", None),
                        policy_version=str(getattr(getattr(req, "policy", None), "policy_version", None) or ""),
                        status=_e2e_map_status(getattr(r, "status", None), getattr(r, "error", None)),
                        service="executor",
                        metadata={"dryrun_finished_ts": time.time(), "path": "dry_only"},
                    )
            except Exception:
                pass
            return r
            ...
            try:
                ...
                if _tid:
                    _e2e_emit(
                        ...
                        status=_e2e_map_status(getattr(r, "status", None), getattr(r, "error", None)),
                        service="executor",
                        metadata={"dryrun_finished_ts": time.time(), "path": "live_disabled"},
                    )
            except Exception:
                pass
            return r
        ...
        try:
            ...
            if _tid:
                _e2e_emit(
                    ...
                    line=getattr(req, "line", None),
                    policy_version=str(getattr(getattr(req, "policy", None), "policy_version", None) or ""),
                    status=_e2e_map_status(getattr(dry, "status", None), getattr(dry, "error", None)),
                    reason=(str(getattr(dry, "error", "") or "")[:240] or None),
                    service="executor",
                    metadata={
                        "dryrun_finished_ts": time.time(),
                        "odd_final": getattr(dry, "odd_final", None),
                        "limit_final": getattr(dry, "limit_final", None),
                    },
                )
        except Exception:
            pass
        ...
        if "H3BUP_vNext" in str(getattr(req.policy, "policy_version", "")):
            # Floor alinhado ao stake live operacional (default 2).
            max_stake = max(float(max_stake), float(os.getenv("EXECUTOR_LIVE_STAKE", "2.0") or 2.0))  # H3BUP_vNext_min_max_stake_live
        ...
        try:
            ...
            if sizing_enabled and req.exec_side == ExecSide.BACK:
                ...
                if "H3BUP_vNext" in str(getattr(req.policy, "policy_version", "")):
                    stake = float(os.getenv("EXECUTOR_LIVE_STAKE", "2.0") or 2.0)  # H3BUP_vNext_force_stake_live
                else:
        ...
        # Gate operacional Back Pre: so permite postar ordem se o slippage medido
                ...
                try:
                    ...
                    if _tid:
                        _e2e_emit(
                            ...
                            policy_version=str(getattr(getattr(req, "policy", None), "policy_version", None) or ""),
                            status="CAP_BLOCKED",
                            reason=str(dry.error or "")[:240],
                            service="executor",
                            metadata={
                                "final_gate_decision_ts": time.time(),
                                "slippage_pre_pct": slip_val,
                                "capacity": cap_val,
                                "odd_at_decision": odd_val,
                                "block_reasons": list(h3bup_block_reasons),
                            },
                        )
                except Exception:
        ...
        # HARD CAP operacional H3BUP: Back Pre LIVE so stake==2; rejeita qualquer outro sizing.
        try:
            ...
            hard_stake = float(os.getenv("EXECUTOR_LIVE_STAKE", "2.0") or 2.0)
            if is_back_pre_hard:
                if (not is_h3b) or (stake is None) or (abs(float(stake) - hard_stake) > 1e-6):
                    dry.status = ExecStatus.CAP_BLOCKED
                    dry.http_status = 200
                    dry.error = f"H3BUP_STAKE_HARD_CAP hard_stake_cap_h3bup_only_2 pol={pol_v!r} stake={stake}"
                    dry.raw = dict(dry.raw or {})
        ...
        try:
            ...
            if _tid:
                ...
                _e2e_emit(
                    ...
                    policy_version=str(getattr(getattr(req, "policy", None), "policy_version", None) or ""),
                    service="executor",
                    metadata={"place_started_ts": time.time(), "stake": stake},
                )
        except Exception:
            pass
        t_place0 = time.time()
        ...
        if place.success:
            ...
            try:
                _tid = _e2e_from_meta(getattr(req, "meta", None))
                _oid_early = _extract_order_id(place.response)
                if _tid:
                    _e2e_emit(
                        "H3B_PLACE_FINISHED",
                        trace_id=_tid,
                        ...
                        status="LIVE_OK",
                        service="executor",
                        duration_ms=float(order_post_ms) if order_post_ms is not None else None,
                        metadata={"place_finished_ts": time.time(), "live_ok_ts": time.time()},
                    )
            except Exception:
                pass
            try:
        ...
        try:
            ...
            if _tid:
                _e2e_emit(
                    ...
                    policy_version=str(getattr(getattr(req, "policy", None), "policy_version", None) or ""),
                    status="API_FAILED",
                    reason=str(dry.error)[:240],
                    service="executor",
                    duration_ms=float(order_post_ms) if order_post_ms is not None else None,
                    metadata={"place_finished_ts": time.time()},
                )
        except Exception:
            pass
