...
def _first_not_none(*vals: Any) -> Any:
    """Retorna o primeiro valor não-None, preservando 0/0.0."""
    for v in vals:
        if v is not None:
            return v
    return None
...
async def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Lista de audit_version separada por vírgula (default: v4.0-api,v5.2-api-back,v5.1-ws-gate-lay)",
    )
    parser.add_argument(
        "--database-url",
        default=None,
    ...
    parser.add_argument(
        "--back-diff-min",
        type=float,
        default=float(os.getenv("BACK_DIFF_MIN", "2.0")),
        help="Corte de edge Back (default: 2.0; use 0 para desativar gate de diff no Back).",
    )
    parser.add_argument(
        "--lay-diff-max",
        type=float,
        default=float(os.getenv("LAY_DIFF_MAX", "-2.0")),
        help="Corte de edge Lay (default: -2.0).",
    )
    ...
    parser.add_argument(
        "--wf-experimental-stats",
        action="store_true",
        default=(str(os.getenv("WF_EXPERIMENTAL_STATS", "0")).strip() in ("1", "true", "True", "yes", "YES")),
        help="Habilita blocos estatísticos exploratórios (bootstrap por key, BH-FDR, testes permutacionais, rolling/sign-test). "
    ...
    daily_turn_back: Dict[str, float] = {}
    daily_turn_lay: Dict[str, float] = {}
    daily_pnl_obs: Dict[str, float] = {}
    daily_pnl_obs_pre: Dict[str, float] = {}
    daily_pnl_obs_in: Dict[str, float] = {}
    daily_pnl_obs_back: Dict[str, float] = {}
    daily_pnl_obs_lay: Dict[str, float] = {}
    daily_pnl_exp: Dict[str, float] = {}
    daily_pnl_exp_pre: Dict[str, float] = {}
    daily_pnl_exp_in: Dict[str, float] = {}
    daily_pnl_exp_back: Dict[str, float] = {}
    daily_pnl_exp_lay: Dict[str, float] = {}
    ...
    wf_exp_stats = bool(getattr(args, "wf_experimental_stats", False))

    # Evidência estatística (OOS; exploratório): acumuladores por match_id, dia, key, liga.
    # Importante: isso NÃO deve rodar no daily 19h por padrão.
    ...
    try:
        ...
        for r in rows:
            ...
            d["lag_total_ms"] = _first_not_none(
                _safe_float(d.get("lag_wall_ms")),
                _safe_float(d.get("audit_total_ms")),
            )
        ...
        if not bool(getattr(args, "walkforward", False)):
            ...
            if allow_back:
                for d0 in oos_any:
                    ...
                    if not _regime_allowed(bool(is_live_eff)):
                        continue
                    h = d0.get("hypothesis_details") or {}
                    slip_pre = _first_not_none(
                        _safe_float(_get_path(h, ["value_sizing", "slippage_pre_pct"])),
                        _safe_float(_get_path(h, ["finance", "value_sizing", "slippage_pre_pct"])),
                        _safe_float(_get_path(h, ["slippage_pre_pct"])),
                    )
                    pre_submit_ms = _first_not_none(
                        _safe_float(_get_path(h, ["value_sizing", "pre_submit_ms"])),
                        _safe_float(_get_path(h, ["finance", "value_sizing", "pre_submit_ms"])),
                        _safe_float(_get_path(h, ["pre_submit_ms"])),
                    )
            ...
            if len(days) < (wf_train + wf_test):
                ...
                for i in ([] if wf_selection_mode == "insample" else range(wf_train, len(days) - wf_test + 1, wf_step)):
                    ...
                    pnl_obs = 0.0
                    # breakdown Back/Lay (turnover + P&L)
                    turn_back = turn_lay = 0.0
                    ...
                    for ev in test_elig:
                        ...
                        if ev.get("side") == "Back":
                            ...
                            turn_back += float(st_eq)
                        else:
                            lay_liab_all += float(exp)
                            turn_lay += float(st_eq)
                    ...
                    if wf_expand:
                        ...
                        scale_lay_pre = (lay_pre_all / lay_pre_roi) if lay_pre_roi > 0 else 1.0
                        scale_back_in = (back_in_all / back_in_roi) if back_in_roi > 0 else 1.0
                        scale_lay_in = (lay_in_all / lay_in_roi) if lay_in_roi > 0 else 1.0

                        pnl_exp_pre = float(pnl_obs_pre_back) * float(scale_back_pre) + float(pnl_obs_pre_lay) * float(scale_lay_pre)
                    ...
                    for dday in test_days:
                        ...
                        daily_turn_back[dday] = daily_turn_back.get(dday, 0.0) + float(turn_back) / max(1, len(test_days))
                        daily_turn_lay[dday] = daily_turn_lay.get(dday, 0.0) + float(turn_lay) / max(1, len(test_days))
                        daily_pnl_obs_back[dday] = daily_pnl_obs_back.get(dday, 0.0) + float(pnl_obs_back) / max(1, len(test_days))
                    ...
                    steps.append(
                        {
                            ...
                            "pnl_obs": pnl_obs,
                            "pnl_exp": pnl_exp,
                            "pnl_obs_back": pnl_obs_back,
                            "pnl_obs_lay": pnl_obs_lay,
                            "pnl_exp_back": pnl_exp_back,
                            "pnl_exp_lay": pnl_exp_lay,
                            "diag": diag,
                            "liq_mode": wf_liq_mode,
                            "liq_scope": wf_liq_scope,
                            "liq_thr_p50_pre": thr_p50_pre,
                ...
                if wf_key_by_league2:
                    ...
                    lines.append("| Train window | Test window | #ativas | Jogos OOS | ROI OOS (mean; IC90) | Turnover (teste) | Turnover Back | Turnover Lay | Lucro (budget) | Lucro Back | Lucro Lay |\n|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
                for s in steps[:20]:
                    if wf_key_by_league2:
                        lines.append(
                            f"| {s['train']} | {s['test']} | {s['active_n']} | {int(s.get('active_n_base') or 0)} | {s['oos_matches']} | {_fmt_pct(s['oos_mean'],2)} {_fmt_ci(s['oos_ci'],2)} | "
                            f"{_fmt_num(s.get('turn_all'),2)} | {_fmt_num(s.get('turn_back'),2)} | {_fmt_num(s.get('turn_lay'),2)} | {_fmt_num(s.get('pnl_exp'),2)} | {_fmt_num(s.get('pnl_exp_back'),2)} | {_fmt_num(s.get('pnl_exp_lay'),2)} |\n"
                        )
                    else:
                        lines.append(
            ...
            if wf_min_m > 0:
                ...
                lines.append(f"| Turnover 30d (Back/Lay) | {_fmt_num(turn_back_30d,2)} / {_fmt_num(turn_lay_30d,2)} |\n")
                ...
                lines.append(f"| Lucro 30d (obs.) Back/Lay | {_fmt_num(profit_obs_back_30d,2)} / {_fmt_num(profit_obs_lay_30d,2)} |\n")
                ...
                lines.append(f"| Lucro 30d (exp.) Back/Lay | {_fmt_num(profit_exp_back_30d,2)} / {_fmt_num(profit_exp_lay_30d,2)} |\n")
                ...
                # Diagnóstico rápido: de onde vem o P&L (Back vs Lay)
                roi_turn_back = (float(profit_exp_back_30d) / float(turn_back_30d) * 100.0) if (profit_exp_back_30d is not None and turn_back_30d and float(turn_back_30d) > 0) else None
                roi_turn_lay = (float(profit_exp_lay_30d) / float(turn_lay_30d) * 100.0) if (profit_exp_lay_30d is not None and turn_lay_30d and float(turn_lay_30d) > 0) else None
                lines.append("**Ablation (OOS): decomposição por lado (Back vs Lay)**\n\n")
                lines.append("| Lado | Turnover 30d | Lucro 30d (exp.) | ROI/turnover 30d (exp.) |\n")
                lines.append("|---|---:|---:|---:|\n")
                ...
                # ------------------------------------------------------------
                # 12.1b+ (EXPLORATÓRIO): evidência estatística / estabilidade / sazonalidade / liga
                # ------------------------------------------------------------
                if not wf_exp_stats:
                    lines.append("### 12.1b Explorações estatísticas (OFF)\n")
                    lines.append(
                        "_Blocos exploratórios (bootstrap por key, BH-FDR, weekday/league perm-test, rolling/sign-test) estão **desligados** por padrão "
                        "para não afetar o relatório diário das 19h. Para habilitar em rodadas manuais: `--wf-experimental-stats` "
                        "ou `WF_EXPERIMENTAL_STATS=1`._\n\n"
                    )
                else:
                    # Helpers (sem custo alto até serem chamados)
                    ...
                    def _weekday_perm_test(day_exp: Dict[str, float], day_pnl: Dict[str, float], *, n_perm: int = 8000) -> Dict[str, Any]:
                        ...
                        if len(days) < 10:
                            return {"n_days": int(len(days)), "p_value": None, "k": 0}
                        ...
                        if len(cats) < 2:
                            return {"n_days": int(len(days)), "p_value": None, "k": int(len(cats))}
                    ...
                    def _empirical_bayes_shrink(means: np.ndarray, vars_over_n: np.ndarray) -> Tuple[float, float, np.ndarray]:
                        ...
                        vbar = float(np.nanmean(vars_over_n)) if vars_over_n.size else 0.0
                    ...
                    lines.append(
                        ...
                        "(placar), e fazemos **bootstrap clusterizado por `match_id`**.\n\n"
                    )
                    ...
                    def _ci_s(bt: dict) -> str:
                        ci = bt.get("ci95") if isinstance(bt.get("ci95"), (list, tuple)) else None
                        if ci and len(ci) == 2:
                            return f"{_fmt_num(ci[0],2)}% .. {_fmt_num(ci[1],2)}%"
                        return "—"
                    lines.append(f"| Bootstrap N matches | {int(bt_all.get('n') or 0)} | {int(bt_back.get('n') or 0)} | {int(bt_lay.get('n') or 0)} |\n")
                    lines.append(f"| Bootstrap ROI_hat | {_fmt_num(bt_all.get('roi_hat'),2)}% | {_fmt_num(bt_back.get('roi_hat'),2)}% | {_fmt_num(bt_lay.get('roi_hat'),2)}% |\n")
                    lines.append(f"| Bootstrap CI95 | {_ci_s(bt_all)} | {_ci_s(bt_back)} | {_ci_s(bt_lay)} |\n")
                    lines.append(
                        f"| Prob(ROI>0) | {_fmt_num(float(bt_all.get('p_roi_gt0') or 0.0)*100.0,1)}% | {_fmt_num(float(bt_back.get('p_roi_gt0') or 0.0)*100.0,1)}% | {_fmt_num(float(bt_lay.get('p_roi_gt0') or 0.0)*100.0,1)}% |\n\n"
                    )

                    # 12.1c — weekday
                    lines.append("### 12.1c Teste: influência por dia da semana (OOS)\n")
                    wd_groups: Dict[str, Dict[str, float]] = {}
                    for dday, ex in (day_obs_exp or {}).items():
                    ...
                    lines.append(f"\n- Teste permutacional (H0: sem diferença): p_value≈`{_fmt_num(pt.get('p_value'),4)}` (n_dias={int(pt.get('n_days') or 0)}, k={int(pt.get('k') or 0)}).\n\n")

                    # 12.1e — por key: bootstrap + FDR + shrinkage
                    lines.append("### 12.1e Evidência por combinação (key): significância + shrinkage\n")
                    key_rows: List[Dict[str, Any]] = []
                    try:
                        rng = np.random.default_rng(24680)
                        for k, mp in (key_match_obs or {}).items():
                            ...
                            if exp_sum <= 0:
                                continue
                            roi_hat = float(pnls.sum() / exp_sum) * 100.0
                            idx = rng.integers(0, len(mids), size=(2500, len(mids)))
                            exp_s = exps[idx].sum(axis=1)
                            pnl_s = pnls[idx].sum(axis=1)
                            roi_s = np.where(exp_s > 0, (pnl_s / exp_s) * 100.0, np.nan)
                            roi_s = roi_s[np.isfinite(roi_s)]
                            if roi_s.size < 200:
                                continue
                            ci = (float(np.quantile(roi_s, 0.025)), float(np.quantile(roi_s, 0.975)))
                            p_le0 = float(np.mean(roi_s <= 0.0))
                            key_rows.append({"key": str(k), "n_matches": int(len(mids)), "exp_sum": exp_sum, "roi_hat": roi_hat, "ci95_lb": ci[0], "ci95_ub": ci[1], "p_roi_le0": p_le0})
                    except Exception:
                        key_rows = []
                    if not key_rows:
                        ...
                        lines.append(
                            ...
                            f"BH‑FDR: m=`{int(fdr.get('m') or 0)}`, cutoff≈`{_fmt_num(fdr.get('cutoff'),4)}`.\n\n"
                        )
                    ...
                    if daily_turn and daily_pnl_exp and len(daily_turn) >= 10:
                        ...
                        t7 = float(sum(float(x[2]) for x in roi_day[-7:]))
                        p7 = float(sum(float(x[3]) for x in roi_day[-7:]))
                        t14 = float(sum(float(x[2]) for x in roi_day[-14:]))
                        p14 = float(sum(float(x[3]) for x in roi_day[-14:]))
                        r7 = (p7 / t7 * 100.0) if t7 > 0 else None
                        r14 = (p14 / t14 * 100.0) if t14 > 0 else None
                        lines.append(f"\n- Rolling 7d: ROI≈`{_fmt_num(r7,2)}%` (P&L={_fmt_num(p7,2)}, Turnover={_fmt_num(t7,2)}).\n")
                        lines.append(f"- Rolling 14d: ROI≈`{_fmt_num(r14,2)}%` (P&L={_fmt_num(p14,2)}, Turnover={_fmt_num(t14,2)}).\n\n")
                    else:
                        lines.append("_Sem série diária suficiente para rolling (precisa >=10 dias)._ \n\n")

                    # 12.1g — efeito de liga no In
                    ...
                    try:
                        ...
                        if not league_rows:
                            ...
                            # perm test em top ligas (compacto)
                            ...
                            if len(xs_roi) >= 80 and len(set(xs_lab)) >= 3:
                                ...
                                for _ in range(n_perm):
                                    ...
                                    if _F_labs(labp) >= f_obs:
                                        cnt += 1
                                p = float((cnt + 1) / (float(n_perm) + 1.0))
                                lines.append(f"\n- Teste permutacional (top ligas; H0: sem diferença): p_value≈`{_fmt_num(p,4)}` (F≈`{_fmt_num(f_obs,2)}`, ligas={len(cats)}, matches={len(roi)}).\n\n")
                            else:
                                lines.append("\n_Sem volume suficiente para teste permutacional entre ligas no in‑match._\n\n")
                    except Exception:
                        lines.append("_Falha ao calcular efeito de liga no in‑match (dados insuficientes)._ \n\n")
