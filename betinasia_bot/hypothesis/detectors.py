...
class DTDownwardTrendEvent:
    ...
    Observação:
    - `odd_current` e o nome correto para DT.
    - `odd_at_reversal` e mantido apenas por compatibilidade com o fluxo historico
      de persistencia, sem implicar reversao de fato.
    """
            ...
            if is_ready and not bool(st.get("signaled_in_streak")):
                ...
                if (now_ts - last_emit) >= float(self.cooldown_sec):
                    ...
                    ev = DTDownwardTrendEvent(
                        ...
                        odd_current=float(curr),
                        odd_at_reversal=float(curr),  # compat payload historico
                        detected_at=now,
                    )
                    st["last_odd"] = curr
                    st["last_ts"] = now
