...
try:
    from ops.h3bup_e2e_trace import (
        ...
        extract_trace_id_from_details as _e2e_from_details,
        attach_trace_meta as _e2e_attach_meta,
        make_trace_id as _e2e_make_trace_id,
    )
except Exception:  # pragma: no cover
    def _e2e_emit(*_a, **_k):
        return None

    def _e2e_from_details(*_a, **_k):
        return None

    def _e2e_attach_meta(meta, *, trace_id, **extra):
        return meta or {}

    def _e2e_make_trace_id(*_a, **_k):
        return None
...
POLICY_ID_H3BUP_VNEXT = "H3BUP_vNext"
POLICY_VERSION_H3BUP_VNEXT = "H3BUP_vNext_20260629"
POLICY_STARTED_AT_H3BUP_VNEXT = "2026-06-29T00:00:00+00:00"
...
def _h3bup_vnext_eval(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Capacidade e slippage final sao medidos de forma confiavel no executor,
    # apos abrir/capturar o betslip. No bridge, bloqueamos apenas o que ja e
    # comparavel antes da execucao: Back Pre e odd na faixa.
...
async def _record_backpre_shadow(db: Database, row: Dict[str, Any], action: str, ev: Dict[str, Any], *, status_operacional: str, execution_id: Optional[str] = None, is_executed: bool = False, non_execution_reason: Optional[str] = None, raw_extra: Optional[Dict[str, Any]] = None) -> None:
    try:
        ...
        params = {
            ...
            "policy_started_at": datetime.fromisoformat(POLICY_STARTED_AT_H3BUP_VNEXT),
...
async def run_bridge(cfg: BridgeConfig) -> int:
    ...
    while True:
        ...
        try:
            ...
            action = f"{cfg.mode}:{cfg.exec_side.value}"
            skey = ""
            _bridge_fetched_ts = time.time()
            _trace_id = None
            try:
                _trace_id = _e2e_from_details(row.get("hypothesis_details"))
                if not _trace_id:
                    _trace_id = _e2e_make_trace_id(src_id)
                row["_e2e_trace_id"] = _trace_id
                row["_bridge_fetched_ts"] = _bridge_fetched_ts
                if cfg.exec_side == ExecSide.BACK:
                    _e2e_emit(
                        "H3B_BRIDGE_FETCHED",
                        trace_id=_trace_id,
                        ...
                        line=row.get("line"),
                        policy_version=POLICY_VERSION_H3BUP_VNEXT,
                        service="executor_bridge_audit",
                        metadata={"audited_at": str(row.get("audited_at") or "")},
                    )
            except Exception:
                pass
            if cfg.exec_side == ExecSide.BACK:
                try:
                    ...
                    pass
                _pe_mono0 = time.monotonic_ns()
                h3bup_vnext_eval = _h3bup_vnext_eval(row)
                try:
                    if _trace_id:
                        _e2e_emit(
                            ...
                            line=row.get("line"),
                            policy_version=POLICY_VERSION_H3BUP_VNEXT,
                            status=("OK" if h3bup_vnext_eval.get("is_h3bup_vnext_pre_exec") else "REJECTED"),
                            reason=h3bup_vnext_eval.get("rejection_reason"),
                            service="executor_bridge_audit",
                            duration_ms=(time.monotonic_ns() - _pe_mono0) / 1e6,
                            metadata={
                                "odd_at_decision": h3bup_vnext_eval.get("odd_at_decision"),
                                "capacity": h3bup_vnext_eval.get("capacity"),
                                "slippage_pre_pct": h3bup_vnext_eval.get("slippage_pre_pct"),
                                "is_h3bup_vnext_pre_exec": h3bup_vnext_eval.get("is_h3bup_vnext_pre_exec"),
                            },
                        )
                except Exception:
                    pass
            ...
            if cfg.exec_side == ExecSide.BACK:
                ...
                if not h3bup_vnext_eval.get("is_h3bup_vnext_pre_exec"):
                    await _mark_seen(
                        db,
                        src_id=src_id,
                        action=action,
            ...
            try:
                ...
                if policy and active_keys is not None:
                    ...
                    if (not ok) and cfg.exec_side == ExecSide.BACK and h3bup_vnext_eval.get("is_h3bup_vnext_pre_exec"):
                        ok = True
                ...
                try:
                    if reserved and _trace_id and cfg.exec_side == ExecSide.BACK:
                        _e2e_emit(
                        ...
                        if (
                            ...
                            if cfg.exec_side == ExecSide.BACK:
                                await _record_backpre_shadow(
                                    ...
                                    h3bup_vnext_eval,
                                    status_operacional="approved_not_executed" if h3bup_vnext_eval.get("is_h3bup_vnext_pre_exec") else "rejected",
                                    non_execution_reason="bankroll",
                                    raw_extra={"balance_current_usd": float(bal_eff), "min_balance_block_usd": float(min_block)},
                                )
                            await _mark_seen(
                                db,
                                src_id=src_id,
                ...
                try:
                    if _trace_id:
                        req.meta = _e2e_attach_meta(
                            req.meta,
                            trace_id=str(_trace_id),
                            audit_id=src_id,
                            bridge_fetched_ts=_bridge_fetched_ts,
                            ws_received_ts=(_details_dict(row).get("ws_received_ts")),
                        )
                except Exception:
                    pass
                try:
                        ...
                        if base_exp is None or float(base_exp) <= 0:
                            if cfg.exec_side == ExecSide.BACK:
                                await _record_backpre_shadow(
                                    ...
                                    h3bup_vnext_eval,
                                    status_operacional="approved_not_executed" if h3bup_vnext_eval.get("is_h3bup_vnext_pre_exec") else "rejected",
                                    non_execution_reason="no_base_exposure",
                                    raw_extra={"base_why": base_why, "odd_hint": odd_hint},
                                )
                            await _mark_seen(
                                db,
                                src_id=src_id,
                        ...
                        if exp_use <= 0:
                            if cfg.exec_side == ExecSide.BACK:
                                await _record_backpre_shadow(
                                    ...
                                    h3bup_vnext_eval,
                                    status_operacional="approved_not_executed" if h3bup_vnext_eval.get("is_h3bup_vnext_pre_exec") else "rejected",
                                    non_execution_reason="budget_blocked",
                                    raw_extra={"bankroll_ref": bankroll_ref, "budget_base": bud_base, "budget_match": bud_match, "cap_signal": cap_signal, "cap_event": cap_event, "spent_before": spent_before, "rem": rem, "rem_event": rem_event, "n_signals_est": n_sig, "risk_mode": risk_mode},
                                )
                            await _mark_seen(
                                db,
                                src_id=src_id,
                ...
                # H3BUP_vNext: policy versionada, sem filtro de ligas, stake live 2.
                try:
                    if cfg.exec_side == ExecSide.BACK and h3bup_vnext_eval.get("is_h3bup_vnext_pre_exec"):
                        req.policy.stake_requested = 2.0
                        req.policy.liability_requested = None
                        ...
                        req.meta["policy"].update({
                            ...
                            "parameters": {"stake": 2.0},
                            "change_reason": "H3BUP_vNext: slippage<0 + odd 1.85-2.15 + capacity>100; stake live=2",
                            "H3BUP_vNext_force_stake_2": True,
                        })
                except Exception:
                    pass
                ...
                try:
                    if _trace_id:
                        _pv = str(getattr(req.policy, "policy_version", None) or "")
                        req.meta = _e2e_attach_meta(
                            req.meta,
                            trace_id=str(_trace_id),
                            audit_id=src_id,
                        ...
                        # Emit AFTER H3BUP policy override so only_h3bup filter accepts the event.
                        _e2e_emit(
                            "H3B_EXEC_REQUEST_CREATED",
                            trace_id=_trace_id,
                            audit_id=src_id,
                            event_id=req.event_id,
                            market_type=str(getattr(req.market_type, "value", req.market_type)),
                            side=req.side,
                            line=req.line,
                            policy_version=_pv or POLICY_VERSION_H3BUP_VNEXT,
                            service="executor_bridge_audit",
                            metadata={"request_created_ts": time.time()},
                        )
                except Exception:
                    pass
                _sent_ts = time.time()
                ...
                try:
                    if _trace_id:
                        _e2e_emit(
                            ...
                            side=req.side,
                            line=req.line,
                            policy_version=str(getattr(req.policy, "policy_version", None) or ""),
                            status=("ACCEPTED" if res.get("accepted") else "REJECTED"),
                            service="executor_bridge_audit",
                            metadata={"request_sent_ts": _sent_ts, "unix_socket": bool(cfg.unix_socket), "accepted": bool(res.get("accepted"))},
                        )
                except Exception:
                    pass
