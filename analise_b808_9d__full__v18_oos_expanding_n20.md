## Atualização do reporte — inclusão de 19/02 e 20/02 (WS-only) + Apêndice (v17)
**Data da atualização:** 20/02/2026 (UTC)  
**Motivo:** incluir os últimos 2 dias (19 e 20/02) no reporte **sem parecer “dias sem apostas”** e sem distorcer métricas OOS baseadas em betslip.

---
### 1) O que mudou na operação em 19–20/02
Desde ~19/02 (fim da tarde UTC), o robô passou a operar em **`v5.0-ws-only`**, onde:
- Não há `betslip_odd` por design (não chama `/v1/betslips/`).
- A métrica central operacional passa a ser a **série WS** por gatilho (t0..t+30), registrada em `hypothesis_details.ws_series`.

Por isso, **essas datas não devem ser interpretadas como “sem apostas/oportunidades”**. Elas são uma mudança de arquitetura (WS-only) visando reduzir slippage e dependência do betslip API.

---
### 2) Resumo objetivo (DB) — 19/02 vs 20/02
Saída observada no banco (query por dia UTC):

| Dia (UTC) | audit_version | OK | Não-OK | Observação |
|---|---|---:|---:|---|
| 19/02 | `v4.0-api` | 580 | 113 | API teve `API_FAILED/STALE/API_BACKOFF/API_RATE_LIMIT` |
| 19/02 | `v5.0-ws-only` | 309 | 0 | WS-only (sem betslip) |
| 20/02 | `v5.0-ws-only` | 466 | 0 | WS-only (sem betslip) |

---
### 3) Qualidade do WS-only (série t0..t+30)
Métrica de completude da série (últimas 24h, `v5.0-ws-only`):

- **n**: 776 eventos
- **ws_pts_max**: 11 (esperado para `0,3,6,...,30`)
- **ws_pts_avg**: 10.99
- **t0_missing_ws_odd**: 0 por hora (t0 sempre presente)

Interpretação operacional:
- A série WS está sendo coletada e persistida corretamente para quase todos os gatilhos.
- Casos com `ws_pts=0` são transitórios (deferred) e tendem a ser patchados para 11 pontos.

---
### 4) OOS walk-forward (expanding) neste documento
O bloco OOS do apêndice (v17) é baseado em **betslip** (diff/CLV/ROI com `betslip_odd`) e por isso **não é diretamente comparável** com o novo regime WS-only.

Neste v18:
- **mantemos** o apêndice OOS do v17 (já com 17/18 excluídos para não distorcer),
- e **incluímos** 19/20 como atualização operacional de WS-only.

---
## Apêndice — Reporte base (v17, com 17/18 excluídos)

> Conteúdo abaixo replicado de `analise_b808_7d__full__v17_oos_expanding_n20.md`.
