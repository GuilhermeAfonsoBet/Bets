...
**Data da execução:** 18/02/2026 11:33 UTC  
...
- **Amostra**: 3946 auditorias (jogos únicos=351, média=11.2 obs/jogo); betslip confiável=1940.
- **Janela efetiva (audited_at)**: 11/02 11:33 → 17/02 21:33 UTC (span≈6.4d; dias com dados=7).
- **Coortes (status=OK, betslip confiável)**: `BS>WS` (diff>=2.0%): **686**; `BS<WS` (diff<=-2.0%): **233**.
- **Coberturas em `hypothesis_details` (OK)**: temporal(back)=1450/1940; lay_temporal=1317/1940; finance=1222/1940.
- **Cobertura de placar (ROI)**: jogos com placar=301/351 (status finished=301).
- **Cobertura de closing_odd (AH)**: jogos com closing=192/351 (54.7%). CLV pre‑match depende disso.
...
- **CLV pre-match (Betslip, API)**: média robusta por jogo +2.126% (IC90 [+1.642%, +2.635%]), com N=921 eventos (jogos=154).
- **Padrão por bucket (CLV PM)**: `BS < WS` -3.928% (sig. negativo), `BS ~ WS` -0.166% (NS), `BS > WS` +6.648% (sig. positivo).
...
| Indicador | Valor |
|---|---:|
| Auditorias H3B `UP` (match + kickoff passado) | 3946 |
| Betslip bruto | 3091 |
| Betslip confiável (diff -10% a +10%) | 1940 |
| Descartados no filtro de qualidade | 1151 |
| Jogos únicos (geral) | 351 |
| Média de observações por jogo | 11.2 |
| Jogos únicos com betslip confiável | 324 |
| Distribuição por market_type | AH=3946 |
| Jogos únicos (AH) no recorte | 351 |
| Jogos únicos (AH) com closing_odd disponível | 192 |
| Cobertura closing_odd (AH) | 54.7% |

---
## 2) Base comparativa: API vs DOM
| Métrica | API (v4.0-api) | DOM (v1.0) |
|---|---:|---:|
| Total de observações | 3946 | 0 |
| Com betslip confiável | 1940 | 0 |
| Com CLV pre-match (betslip) | 921 | 0 |
| Com ROI (betslip) | 1725 | 0 |
| Tempo total observado (detecção→betslip, wall/total) | 15258 ms | — ms |
| Tempo instrumentado (detecção→clique→betslip) | 8282 ms | — ms |
...
|---|---|---:|---:|---:|---:|
| API (2-4s) | lag_det→click | 5463 | 807 | 6268 | 3946 |
| API (2-4s) | lag_click→betslip | 2601 | 2138 | 4354 | 3655 |
| API (2-4s) | lag_e2e (soma) | 8282 | 3402 | 9302 | 3655 |
| API (2-4s) | audit_total (duração) | 15254 | 4741 | 51996 | 3946 |
| API (2-4s) | overhead (total - e2e) | 7729 | 257 | 30578 | 3655 |
...
|---|---:|---:|---|
| Observações totais com classificação temporal | 1703 | 2243 | Contagem bruta do corte |
| ROI Betslip | 998 | 727 | Amostra com resultado do jogo |
| ROI WebSocket | 1564 | 1960 | Referência de mercado |
| CLV (apenas pre-match) | 921 | — | CLV vs closing pré-jogo não é interpretável in-match |

---
### 2.2 Performance por regime (pre-match vs in-match)
| Regime | N auditorias | N betslip conf. | N OK | Back edge | Lay edge | Diff médio (OK) |
|---|---:|---:|---:|---:|---:|---:|
| PRE_MATCH | 1703 | 1089 | 1089 | 422 | 75 | +2.187% |
| IN_MATCH | 2243 | 851 | 851 | 264 | 158 | +0.987% |
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| < 5s | 1285 | 273 | 3561 | 4841 | 2504 | 427 | 136 | +2.34% [+1.89%, +2.79%] | +3.52% [-2.36%, +9.50%] |
| 5-10s | 232 | 145 | 5913 | 8388 | 3086 | 95 | 46 | +1.78% [+0.84%, +2.73%] | -2.84% [-14.58%, +9.11%] |
| 10-20s | 22 | 20 | 13985 | 19116 | 13947 | 9 | 5 | +1.17% [-1.58%, +3.89%] | +10.43% [-23.26%, +43.02%] |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| < 5s | 1285 | 427 | 136 | +7.04% [+6.60%, +7.46%] | -3.58% [-4.78%, -2.36%] | +4.06% [-5.91%, +14.28%] | -1.70% [-16.51%, +13.70%] |
| 5-10s | 232 | 95 | 46 | +5.65% [+4.74%, +6.49%] | -5.91% [-7.17%, -4.54%] | -5.65% [-23.36%, +12.05%] | -10.04% [-29.31%, +10.09%] |
| 10-20s | 22 | 9 | 5 | +3.86% [+1.34%, +6.62%] | -6.98% — | -10.16% [-55.47%, +36.27%] | +40.04% [+0.00%, +79.90%] |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 2026-02-11 | API (2-4s) | 186 | 64 | 36.6% | 17.7% | 26432 | +7.02% | -4.08% |
...
| Métrica | API | DOM |
|---|---:|---:|
| CLV Bruto BS Pre-Match | +2.320% (sig. positivo, N=921, jogos=154) | — (N/A, N=0, jogos=0) |
| CLV Adicional BS Pre-Match | +1.795% (sig. positivo, N=921, jogos=154) | — (N/A, N=0, jogos=0) |
| Taxa de CLV > 0 (bruto) | 60.4% | —% |
| Taxa de CLV > 0 (adicional) | 61.9% | —% |

Notas de robustez (IC 90% por jogo):  
- API CLV bruto (cluster): média +2.126%; IC90 [+1.642%, +2.635%]  
...
| Métrica | API | DOM |
|---|---:|---:|
| ROI Betslip | +1.339% (NS, N=1723) | — (N/A, N=0) |
| ROI WebSocket | -0.691% (NS, N=3508) | — (N/A, N=0) |
| Win rate ROI Betslip | 50.7% | —% |
| Win rate ROI WS | 50.0% | —% |

Notas de robustez (IC 90% por jogo):  
- API ROI betslip (cluster): média +1.249%; IC90 [-3.601%, +6.198%]  
- API ROI WS (cluster): média -2.858%; IC90 [-5.701%, +0.033%]  

---
## 5) Diferença de preço BS vs WS
| Métrica | API | DOM |
|---|---:|---:|
| Diff BS vs WS (média) | +1.661% (sig. positivo, N=1940) | — (N/A, N=0) |
| BS > WS | 47.2% (916/1940) | —% (0/0) |
| BS > WS +2% | 35.4% (686/1940) | —% (0/0) |
...
|---|---:|---:|---|---:|---:|---:|---|
| BS < WS (-10% a -2%) | 233 | -3.928% | [-4.914%, -3.049%] | 58 | 45 | +2.056% | [-13.912%, +8.687%] |
| BS ~ WS (-2% a +2%) | 1021 | -0.166% | [-0.620%, +0.113%] | 495 | 135 | +0.146% | [-10.115%, +2.231%] |
| BS > WS (+2% a +10%) | 686 | +6.648% | [+6.467%, +7.288%] | 368 | 115 | +2.847% | [-1.544%, +14.934%] |

---
### 6.2 Combinação por faixa de linha AH
| Faixa AH | CLV BS PM (média) | IC90 (cluster) | ROI BS (todos) | IC90 (cluster) | Diff BS vs WS (média) |
|---|---:|---|---:|---|---:|
| AH 0-1 (líquida) | +2.557% | [+2.153%, +3.563%] | +0.993% | [+0.283%, +14.035%] | +1.898% |
| AH 1-2 (média) | +2.287% | [+1.169%, +3.401%] | +0.791% | [-13.118%, +12.148%] | +2.348% |
| AH 2+ (extrema) | +1.949% | [+0.247%, +1.947%] | +1.857% | [-7.242%, +7.992%] | +1.202% |
...
|---|---:|---|---:|---:|---:|---|---:|
| < 10s | +2.367% | [+1.701%, +2.594%] | 698 | 143 | +1.412% | [-3.465%, +7.805%] | +1.612% |
| 10-20s | +1.220% | [-1.579%, +3.893%] | 7 | 7 | +9.839% | [-23.261%, +43.022%] | +1.059% |
...
| stake_cap | 0.00 |
| Cobertura finance (OK, betslip conf.) | 1222/1940 |
...
| Corte (diff_pct) | >= 2.0% |
| N eventos | 686 |
| Cobertura finance (na coorte) | 426/686 |
| Stake total (estimado) | 218680.53 |
| Stake médio | 318.78 |
| Profit_if_win total (estimado) | 234850.34 |
| Profit_if_win médio | 342.35 |
| N com ROI realizado | 620 |
| P&L realizado total (estimado) | -42959.49 |
| ROI realizado (ponderado por stake) | -20.21% |
| ROI realizado (robusto por jogo, mean; IC90) | +6.73% [-1.54%, +14.93%] |
| ROI ponderado por stake (robusto por jogo, mean; IC90) | +5.83% [-4.49%, +16.39%] |
...
| Corte (diff_pct) | <= -2.0% |
| N eventos | 233 |
| Cobertura finance (na coorte) | 144/233 |
| Stake total (estimado) | 41672.94 |
| Liability total (estimada) | 36458.64 |
| Liability média | 156.47 |
| Liability p95 | 562.33 |
| Liability p99 | 2032.55 |
| ES95 (liability) | 1663.71 |
| Liability max | 4395.35 |
| Proxy de banca (>= p99 liability) | 2032.55 |
| N com ROI realizado | 150 |
| P&L realizado total (estimado) | -2911.54 |
| ROI realizado (ponderado por liability) | -8.60% |
| ROI realizado (ponderado por stake) | -7.50% |
| ROI/liability (robusto por jogo, mean; IC90) | +13.28% [-1.31%, +28.08%] |
| ROI/liability ponderado (robusto por jogo, mean; IC90) | +14.56% [-0.88%, +30.01%] |
...
|---|---:|---:|---:|---:|
| Back | 7.0 | 937202.29 | -184112.11 | -189452.64 |
| Lay (stake) | 7.0 | 178598.29 | -12478.04 | -13389.48 |
| Total (Back+Lay) | 7.0 | 1115800.58 | -196590.16 | -202842.12 |
...
|---|---:|---:|---:|---:|
| Back (stake) | 4395.20 | 3865.79 | -4188.94% | -4310.45% |
| Lay (liability) | 2032.55 | 1663.71 | -613.91% | -658.75% |
| Total (soma) | 6427.75 | 5529.50 | -3058.46% | -3155.73% |
...
|---|---:|---:|---:|---:|---:|
| Back (stake) | 16474.16 | 45197.64 | 73981.00 | 80107.42 | 81379.10 |
| Lay (liability) | 2122.03 | 6134.07 | 8877.35 | 9203.58 | 9765.09 |
| Total (Back+Lay) | 18574.91 | 47541.78 | 80143.03 | 86250.51 | 88157.33 |

**Banca recomendada (conservadora)**

| Métrica | Valor |
|---|---:|
| Banca por risco (p99 unitário, soma) | 6427.75 |
| Banca por liquidez (p99 simultâneo + buffer) | 88157.33 |
| Banca efetiva (max das duas) | 88157.33 |
| ROI/banca 30d (direto, banca efetiva) | -223.00% |
| ROI/banca 30d (ROI×turnover, banca efetiva) | -230.09% |
**Diagnóstico de cobertura (placar/ROI)**

| Bloco | Turnover total | Turnover com ROI | Cobertura turnover |
|---|---:|---:|---:|
| Back | 218680.53 | 212516.09 | 97.18% |
| Lay | 41672.94 | 38836.21 | 93.19% |

Notas (Lay): exposição 30d por liability (não é turnover) = 156251.32; ROI realizado por liability (ponderado) = -8.60%.
...
|---|---:|---:|---:|---:|---:|---:|---:|
| PRE_MATCH | 1089 | 4.1 | 5.2 | 78.2% | 15.5% | 12.2 | 7.2 |
| IN_MATCH | 851 | 4.9 | 0.0 | 63.6% | 28.6% | 13.5 | 8.1 |
...
|---|---:|---:|---:|---:|
| PRE_MATCH | 81.6% | 6.5% | 9.0% | 2.8% |
| IN_MATCH | 69.0% | 4.5% | 24.1% | 2.5% |
...
|---|---:|---:|---:|---:|---:|
| t+0s | 1940 | +1.66% | 2.013 | +2.32% | 2.03 |
...
|---|---:|---:|---:|---:|---:|
| SEM_REVERSAO | 1528 | 773 | +1.78% [+1.32%, +2.24%] | +2.04% [+1.59%, +2.50%] | +2.03% [+1.58%, +2.49%] |
...
|---|---:|---:|---:|---:|---:|
| SEM_REVERSAO | 1528 | 1365 | +1.23% [-4.40%, +7.00%] | +1.37% [-4.33%, +7.28%] | +1.35% [-4.35%, +7.26%] |
...
|---|---:|---:|---:|---:|
| SEM_REVERSAO | 773 | 1.980 [+1.969, +1.991] | 1.987 [+1.975, +1.998] | 1.966 [+1.958, +1.975] |
...
| Back | Pre | No | 920 | 198 | +1.78% [+1.32%, +2.24%] | -4.06% [-11.27%, +3.11%] | -6.40% | não (CLV p90>0 AND ROI>0) |
| Back | In | Yes | 243 | 122 | — | +1.86% [-12.61%, +17.04%] | -2.67% | não (ROI p30>0) |
| Back | In | No | 608 | 219 | — | +3.88% [-4.01%, +11.77%] | +1.35% | sim (ROI p30>0) |
| Lay | Pre | Yes | 189 | 99 | -0.25% [-1.19%, +0.72%] | +4.27% [-9.10%, +17.98%] | +0.05% | não (CLV p90<0 AND ROI p30>0) |
| Lay | Pre | No | 754 | 183 | +0.49% [-0.15%, +1.13%] | +3.56% [-4.03%, +11.24%] | +1.22% | não (CLV p90<0 AND ROI p30>0) |
| Lay | In | Yes | 263 | 138 | — | +30.94% [+10.75%, +54.06%] | +23.98% | sim (ROI p30>0) |
...
| PRE_MATCH | AH 2+ (extrema) | < 10s | 88 | 40 | +6.96% | [+6.57%, +7.31%] |
| PRE_MATCH | AH 1-2 (média) | < 10s | 79 | 30 | +6.68% | [+6.59%, +7.37%] |
| IN_MATCH | AH 0-1 (líquida) | < 10s | 46 | 38 | +6.61% | [+5.99%, +7.14%] |
...
| PRE_MATCH | AH 0-1 (líquida) | < 10s | 23 | 20 | -4.65% | [-5.84%, -4.06%] | 3965.77 |
...
- **Lay (liability)**: corr(exposição, ROI)=0.021; corr(exposição, CLV)=0.063 (onde CLV existe).
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Back | FLAT | 620 | 620.00 | 24.12 | 3.89% | 1.00 | 1.00 | 6.12 | 10.53 |
| Lay | FLAT | 201 | 255.98 | -2.54 | -0.99% | 1.00 | 1.00 | 32.14 | 58.84 |
| Back | PROXY | 620 | 212516.09 | -42959.49 | -20.21% | 4398.50 | 4136.53 | 185690.76 | 254793.87 |
| Lay | PROXY | 150 | 38836.21 | -2911.54 | -7.50% | 3260.98 | 2162.72 | 23429.54 | 41772.22 |
| Back | KELLY_0.10 | 326 | 8539.83 | 440.18 | 5.15% | 57.48 | 52.52 | 496.43 | 909.02 |
| Lay | KELLY_0.10 | 46 | 602.69 | 85.05 | 14.11% | 20.33 | 20.33 | 27.26 | 47.64 |
| Back | KELLY_0.25 | 326 | 17363.35 | 499.37 | 2.88% | 87.90 | 87.90 | 1182.57 | 2246.83 |
| Lay | KELLY_0.25 | 46 | 865.55 | 142.45 | 16.46% | 20.33 | 20.33 | 28.44 | 50.29 |
| Back | KELLY_0.50 | 326 | 21164.33 | 363.02 | 1.72% | 87.90 | 87.90 | 1257.76 | 2497.14 |
| Lay | KELLY_0.50 | 46 | 950.70 | 130.64 | 13.74% | 20.33 | 20.33 | 20.54 | 35.35 |
| Back | KELLY_1.00 | 326 | 21855.01 | 339.52 | 1.55% | 87.90 | 87.90 | 1405.47 | 2762.93 |
| Lay | KELLY_1.00 | 46 | 965.75 | 139.65 | 14.46% | 20.33 | 20.33 | 13.07 | 22.96 |
...
| Back | Pre | Yes | KELLY_0.10 | 80 | 2252.42 | -93.69 | -4.16% | 47.98 | 1563.32 |
| Back | Pre | Yes | KELLY_0.25 | 80 | 4799.18 | -79.01 | -1.65% | 87.90 | 2031.73 |
| Back | Pre | Yes | KELLY_0.50 | 80 | 5767.81 | 17.81 | 0.31% | 87.90 | 1728.78 |
| Back | Pre | Yes | KELLY_1.00 | 80 | 5894.52 | -14.73 | -0.25% | 87.90 | 2130.90 |
| Back | Pre | No | FLAT | 305 | 305.00 | 4.14 | 1.36% | 1.00 | 51.05 |
| Back | Pre | No | PROXY | 305 | 133236.49 | -24000.07 | -18.01% | 4488.02 | 171260.24 |
| Back | Pre | No | KELLY_0.10 | 246 | 6287.41 | 533.87 | 8.49% | 57.35 | 567.42 |
| Back | Pre | No | KELLY_0.25 | 246 | 12564.17 | 578.38 | 4.60% | 87.90 | 1451.85 |
| Back | Pre | No | KELLY_0.50 | 246 | 15396.51 | 345.21 | 2.24% | 87.90 | 1864.71 |
| Back | Pre | No | KELLY_1.00 | 246 | 15960.48 | 354.25 | 2.22% | 87.90 | 1843.17 |
| Back | In | Yes | FLAT | 58 | 58.00 | -4.52 | -7.80% | 1.00 | 49.71 |
| Back | In | Yes | PROXY | 58 | 9973.72 | -3099.84 | -31.08% | 1457.62 | 34130.06 |
| Back | In | No | FLAT | 166 | 166.00 | 24.84 | 14.96% | 1.00 | 0.00 |
| Back | In | No | PROXY | 166 | 38360.55 | -7022.96 | -18.31% | 2328.92 | 59669.47 |
| Lay | Pre | Yes | FLAT | 13 | 14.38 | 3.45 | 24.02% | 1.00 | 7.00 |
| Lay | Pre | Yes | PROXY | 13 | 6138.31 | -3626.31 | -59.08% | 3925.76 | 40750.69 |
| Lay | Pre | Yes | KELLY_0.10 | 9 | 96.39 | 16.28 | 16.89% | 19.97 | 116.95 |
| Lay | Pre | Yes | KELLY_0.25 | 9 | 148.74 | 35.03 | 23.55% | 20.33 | 101.63 |
| Lay | Pre | Yes | KELLY_0.50 | 9 | 178.97 | 41.42 | 23.14% | 20.33 | 101.63 |
| Lay | Pre | Yes | KELLY_1.00 | 9 | 181.24 | 43.70 | 24.11% | 20.33 | 99.07 |
| Lay | Pre | No | FLAT | 34 | 38.88 | 0.74 | 1.91% | 1.00 | 6.81 |
| Lay | Pre | No | PROXY | 34 | 10447.02 | -3673.57 | -35.16% | 3340.84 | 31418.67 |
| Lay | Pre | No | KELLY_0.10 | 25 | 370.36 | 12.78 | 3.45% | 20.33 | 46.04 |
| Lay | Pre | No | KELLY_0.25 | 25 | 517.45 | 34.64 | 6.69% | 20.33 | 28.48 |
| Lay | Pre | No | KELLY_0.50 | 25 | 557.13 | 18.26 | 3.28% | 20.33 | 84.48 |
| Lay | Pre | No | KELLY_1.00 | 25 | 557.13 | 18.26 | 3.28% | 20.33 | 84.59 |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| BackFast (<5s, PM) + LayReversal (t_ext<=3s) | FLAT | 259 | 25 | 1110 | 107 | 1.00 | 1.10 | 1.00 | 1227.62 | 77.01 | 6.27% | 15.85% | 14.44% | 1.00 | 1.00 | 2.00 | 160.60 | 160.60 | 47.95% | 6383.60 | 400.47 | 835.12 | 33.37 |
| BackFast (<5s, PM) + LayReversal (t_ext<=3s) | KELLY_0.25 | 216 | 21 | 926 | 90 | 54.41 | 17.28 | 15.25 | 51927.77 | 3935.99 | 7.58% | 15.68% | 13.84% | 87.90 | 20.33 | 108.23 | 7022.40 | 7022.40 | 56.05% | 270024.40 | 20467.17 | 36516.46 | 1410.82 |
...
**Escala Kelly usada nesta curva**: P99_PROXY | ref_back=4395.20 ref_lay=2032.55 | cap_back=2.0% cap_lay=1.0% | max_stake_event=100%*limit
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
| BackFast+LayReversal | KELLY_0.10 | 0.0% | 16.5% | 4.2% | 8.3% | 25392.59 | 2230.01 | 67.29% | 636.94 |
| BackFast+LayReversal | KELLY_0.25 | 22.7% | 31.0% | 58.3% | 8.3% | 51927.77 | 3935.99 | 56.05% | 1383.41 |
| BackFast+LayReversal | KELLY_0.50 | 81.0% | 36.4% | 87.5% | 12.5% | 63729.65 | 4242.71 | 48.63% | 1059.64 |
| BackFast+LayReversal | KELLY_1.00 | 90.5% | 36.8% | 100.0% | 12.5% | 65354.43 | 4668.36 | 51.91% | 1090.03 |
...
|---|---|---:|---:|---:|---:|
| IN_MATCH BackFast (<5s) | FLAT | 128 | 128.00 | 18.59 | 14.52% |
| IN_MATCH BackFast (<5s) | PROXY | 128 | 25080.04 | -1868.58 | -7.45% |
...
| Filtro | Jogos únicos |
|---|---:|
| Combinações elegíveis (edge + timing + t0) | 261 |
| Com ROI disponível (precisa de placar) | 231 |
...
|---|---|---:|---:|---:|---:|---:|---:|
| 2026-02-11→2026-02-13 | 2026-02-14→2026-02-14 | 1 | 44 | +13.53% [-10.44%, +39.86%] | 16086.41 | -5006.86 | -5006.86 |
| 2026-02-12→2026-02-14 | 2026-02-15→2026-02-15 | 3 | 56 | +15.77% [-3.07%, +34.20%] | 12220.98 | -536.04 | -570.72 |
...
| Dias OOS usados | 2 |
| Turnover 30d (proj.) | 424610.93 |
| Turnover 30d (Pre/In) | 40845.11 / 383765.81 |
| Lucro 30d (obs.) | -83143.59 |
| Lucro 30d (obs.) Pre/In | 2681.45 / -85825.03 |
| Lucro 30d (exp.) | -83663.79 |
| Lucro 30d (exp.) Pre/In | 2681.45 / -86734.45 |
| Banca risco p99 (Back+Lay) | 16047.76 |
| Banca liquidez p99 (+buf) | 12625.03 |
| Banca recomendada (max) | 16047.76 |
| ROI/banca 30d (obs.) | -518.10% |
| ROI/banca 30d (exp.) | -521.34% |
| DD 30d p95 (obs.) | 105497.67 |
| DD 30d p95 (exp.) | 105844.47 |
...
Referência de banca p/ budget: 16047.76 | budgets por jogo aplicados em stake (Back) e liability (Lay).

| Cenário | Turnover 30d | Lucro 30d (exp.) | Banca rec. (max) | ROI/banca 30d (exp.) | DD 30d p95 (exp.) |
|---|---:|---:|---:|---:|---:|
| BASELINE (sem budget) | 424610.93 | -83663.79 | 16047.76 | -521.34% | 105844.47 |
| BUDGET_0.50%/0.25% cap25% | 38302.68 | 3804.44 | 1588.28 | 239.53% | 653.17 |
| BUDGET_1.00%/0.50% cap33% | 86358.90 | 8286.71 | 3525.35 | 235.06% | 1630.85 |
| BUDGET_2.00%/1.00% cap50% | 182834.26 | 16014.63 | 6583.23 | 243.26% | 6465.30 |
...
| Indicador | Valor |
|---|---:|
| Jogos únicos no recorte | 351 |
| Jogos com placar disponível (home_score/away_score não nulos) | 301 |
| Jogos com status='finished' no banco | 301 |
