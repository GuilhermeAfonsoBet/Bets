...
**Data da execução:** 18/02/2026 12:46 UTC  
...
- **Amostra**: 3917 auditorias (jogos únicos=344, média=11.4 obs/jogo); betslip confiável=1917.
- **Janela efetiva (audited_at)**: 11/02 12:50 → 17/02 21:33 UTC (span≈6.4d; dias com dados=7).
- **Coortes (status=OK, betslip confiável)**: `BS>WS` (diff>=2.0%): **681**; `BS<WS` (diff<=-2.0%): **228**.
- **Coberturas em `hypothesis_details` (OK)**: temporal(back)=1450/1917; lay_temporal=1317/1917; finance=1222/1917.
- **Cobertura de placar (ROI)**: jogos com placar=299/344 (status finished=299).
- **Cobertura de closing_odd (AH)**: jogos com closing=192/344 (55.8%). CLV pre‑match depende disso.
...
| Indicador | Valor |
|---|---:|
| Auditorias H3B `UP` (match + kickoff passado) | 3917 |
| Betslip bruto | 3062 |
| Betslip confiável (diff -10% a +10%) | 1917 |
| Descartados no filtro de qualidade | 1145 |
| Jogos únicos (geral) | 344 |
| Média de observações por jogo | 11.4 |
| Jogos únicos com betslip confiável | 318 |
| Distribuição por market_type | AH=3917 |
| Jogos únicos (AH) no recorte | 344 |
| Jogos únicos (AH) com closing_odd disponível | 192 |
| Cobertura closing_odd (AH) | 55.8% |

---
## 2) Base comparativa: API vs DOM
| Métrica | API (v4.0-api) | DOM (v1.0) |
|---|---:|---:|
| Total de observações | 3917 | 0 |
| Com betslip confiável | 1917 | 0 |
| Com CLV pre-match (betslip) | 921 | 0 |
| Com ROI (betslip) | 1714 | 0 |
| Tempo total observado (detecção→betslip, wall/total) | 15336 ms | — ms |
| Tempo instrumentado (detecção→clique→betslip) | 8330 ms | — ms |
...
|---|---|---:|---:|---:|---:|
| API (2-4s) | lag_det→click | 5497 | 808 | 6404 | 3917 |
| API (2-4s) | lag_click→betslip | 2610 | 2146 | 4376 | 3626 |
| API (2-4s) | lag_e2e (soma) | 8330 | 3417 | 9399 | 3626 |
| API (2-4s) | audit_total (duração) | 15332 | 4742 | 52394 | 3917 |
| API (2-4s) | overhead (total - e2e) | 7773 | 257 | 30758 | 3626 |
...
|---|---:|---:|---|
| Observações totais com classificação temporal | 1688 | 2229 | Contagem bruta do corte |
| ROI Betslip | 993 | 721 | Amostra com resultado do jogo |
| ROI WebSocket | 1557 | 1952 | Referência de mercado |
...
|---|---:|---:|---:|---:|---:|---:|
| PRE_MATCH | 1688 | 1076 | 1076 | 420 | 71 | +2.218% |
| IN_MATCH | 2229 | 841 | 841 | 261 | 157 | +0.976% |
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| < 5s | 1268 | 269 | 3576 | 4842 | 2503 | 422 | 135 | +2.34% [+1.89%, +2.79%] | +2.88% [-3.03%, +8.80%] |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| < 5s | 1268 | 422 | 135 | +7.04% [+6.60%, +7.46%] | -3.58% [-4.78%, -2.36%] | +2.98% [-7.18%, +13.00%] | -1.70% [-16.51%, +13.70%] |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 2026-02-11 | API (2-4s) | 163 | 58 | 38.7% | 17.2% | 27058 | +7.02% | -4.08% |
...
| Métrica | API | DOM |
|---|---:|---:|
| ROI Betslip | +1.219% (NS, N=1712) | — (N/A, N=0) |
| ROI WebSocket | -0.659% (sig. negativo, N=3493) | — (N/A, N=0) |
| Win rate ROI Betslip | 50.6% | —% |
| Win rate ROI WS | 50.1% | —% |

Notas de robustez (IC 90% por jogo):  
- API ROI betslip (cluster): média +0.684%; IC90 [-4.126%, +5.626%]  
- API ROI WS (cluster): média -2.770%; IC90 [-5.520%, -0.019%]  

---
## 5) Diferença de preço BS vs WS
| Métrica | API | DOM |
|---|---:|---:|
| Diff BS vs WS (média) | +1.673% (sig. positivo, N=1917) | — (N/A, N=0) |
| BS > WS | 47.5% (910/1917) | —% (0/0) |
| BS > WS +2% | 35.5% (681/1917) | —% (0/0) |
...
|---|---:|---:|---|---:|---:|---:|---|
| BS < WS (-10% a -2%) | 228 | -3.928% | [-4.914%, -3.049%] | 58 | 45 | +2.067% | [-14.206%, +8.338%] |
| BS ~ WS (-2% a +2%) | 1008 | -0.166% | [-0.620%, +0.113%] | 495 | 135 | +0.169% | [-9.767%, +2.703%] |
| BS > WS (+2% a +10%) | 681 | +6.648% | [+6.467%, +7.288%] | 368 | 115 | +2.475% | [-2.187%, +14.478%] |
...
| AH 1-2 (média) | +2.287% | [+1.169%, +3.401%] | +0.791% | [-13.118%, +12.148%] | +2.379% |
| AH 2+ (extrema) | +1.949% | [+0.247%, +1.947%] | +1.778% | [-7.432%, +8.458%] | +1.204% |
...
|---|---:|---|---:|---:|---:|---|---:|
| < 10s | +2.367% | [+1.701%, +2.594%] | 698 | 143 | +1.259% | [-4.237%, +7.624%] | +1.617% |
...
| stake_cap | 0.00 |
| Cobertura finance (OK, betslip conf.) | 1222/1917 |
...
| Corte (diff_pct) | >= 2.0% |
| N eventos | 681 |
| Cobertura finance (na coorte) | 426/681 |
| Stake total (estimado) | 217919.22 |
| Stake médio | 320.00 |
| Profit_if_win total (estimado) | 233908.89 |
| Profit_if_win médio | 343.48 |
| N com ROI realizado | 616 |
| P&L realizado total (estimado) | -43606.79 |
| ROI realizado (ponderado por stake) | -20.57% |
| ROI realizado (robusto por jogo, mean; IC90) | +6.01% [-2.19%, +14.48%] |
| ROI ponderado por stake (robusto por jogo, mean; IC90) | +5.00% [-5.11%, +15.61%] |
...
| Corte (diff_pct) | <= -2.0% |
| N eventos | 228 |
| Cobertura finance (na coorte) | 144/228 |
| Stake total (estimado) | 41313.42 |
| Liability total (estimada) | 36116.01 |
| Liability média | 158.40 |
| Liability p95 | 566.46 |
| Liability p99 | 2041.46 |
| ES95 (liability) | 1663.71 |
| Liability max | 4395.35 |
| Proxy de banca (>= p99 liability) | 2041.46 |
| N com ROI realizado | 149 |
| P&L realizado total (estimado) | -2911.54 |
| ROI realizado (ponderado por liability) | -8.61% |
| ROI realizado (ponderado por stake) | -7.50% |
| ROI/liability (robusto por jogo, mean; IC90) | +13.69% [-1.01%, +28.47%] |
| ROI/liability ponderado (robusto por jogo, mean; IC90) | +14.76% [-0.72%, +30.20%] |
...
|---|---:|---:|---:|---:|
| Back | 7.0 | 933939.51 | -186886.24 | -192113.12 |
| Lay (stake) | 7.0 | 177057.50 | -12478.04 | -13279.53 |
| Total (Back+Lay) | 7.0 | 1110997.01 | -199364.28 | -205392.65 |
...
|---|---:|---:|---:|---:|
| Back (stake) | 4395.45 | 3865.79 | -4251.81% | -4370.73% |
| Lay (liability) | 2041.46 | 1663.71 | -611.23% | -650.49% |
| Total (soma) | 6436.91 | 5529.50 | -3097.21% | -3190.86% |
...
|---|---:|---:|---:|---:|---:|
| Back (stake) | 19033.35 | 58273.98 | 83753.65 | 88385.64 | 92129.02 |
| Lay (liability) | 2564.92 | 7184.50 | 9539.93 | 11582.29 | 10493.92 |
| Total (Back+Lay) | 21564.10 | 62989.36 | 89954.67 | 95513.70 | 98950.14 |

**Banca recomendada (conservadora)**

| Métrica | Valor |
|---|---:|
| Banca por risco (p99 unitário, soma) | 6436.91 |
| Banca por liquidez (p99 simultâneo + buffer) | 98950.14 |
| Banca efetiva (max das duas) | 98950.14 |
| ROI/banca 30d (direto, banca efetiva) | -201.48% |
| ROI/banca 30d (ROI×turnover, banca efetiva) | -207.57% |
**Diagnóstico de cobertura (placar/ROI)**

| Bloco | Turnover total | Turnover com ROI | Cobertura turnover |
|---|---:|---:|---:|
| Back | 217919.22 | 211990.22 | 97.28% |
| Lay | 41313.42 | 38819.93 | 93.96% |

Notas (Lay): exposição 30d por liability (não é turnover) = 154782.88; ROI realizado por liability (ponderado) = -8.61%.
...
|---|---:|---:|---:|---:|---:|---:|---:|
| PRE_MATCH | 1076 | 4.2 | 5.2 | 78.0% | 15.7% | 12.2 | 7.2 |
...
|---|---:|---:|---:|---:|---:|
| t+0s | 1917 | +1.67% | 2.013 | +2.32% | 1.91 |
...
|---|---:|---:|---:|---:|---:|
| SEM_REVERSAO | 1505 | 773 | +1.78% [+1.32%, +2.24%] | +2.04% [+1.59%, +2.50%] | +2.03% [+1.58%, +2.49%] |
...
|---|---:|---:|---:|---:|---:|
| SEM_REVERSAO | 1505 | 1354 | +0.61% [-5.10%, +6.20%] | +0.76% [-5.05%, +6.36%] | +0.74% [-5.07%, +6.33%] |
...
|---|---:|---:|---:|---:|
| SEM_REVERSAO | 773 | 1.983 [+1.973, +1.994] | 1.990 [+1.979, +2.001] | 1.966 [+1.958, +1.975] |
...
| Back | Pre | No | 907 | 193 | +1.78% [+1.32%, +2.24%] | -4.40% [-11.62%, +2.79%] | -6.62% | não (CLV p90>0 AND ROI>0) |
| Back | In | Yes | 243 | 122 | — | +1.86% [-12.61%, +17.04%] | -2.67% | não (ROI p30>0) |
| Back | In | No | 598 | 216 | — | +3.37% [-4.75%, +11.64%] | +0.71% | sim (ROI p30>0) |
...
| PRE_MATCH | AH 2+ (extrema) | < 10s | 86 | 39 | +6.99% | [+6.59%, +7.35%] |
...
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|
|
| Lay | Pre | No | KELLY_0.25 | 25 | 519.50 | 34.84 | 6.71% | 20.41 | 28.29 |
| Lay | Pre | No | KELLY_0.50 | 25 | 559.35 | 18.39 | 3.29% | 20.41 | 84.31 |
| Lay | Pre | No | KELLY_1.00 | 25 | 559.35 | 18.39 | 3.29% | 20.41 | 84.55 |
...
| Back | Pre | No | 907 | 193 | +1.78% [+1.32%, +2.24%] | -4.40% [-11.62%, +2.79%] | -6.62% | pre: Kelly OK |
| Back | In | Yes | 243 | 122 | — — | +1.86% [-12.61%, +17.04%] | -2.67% | in: use FLAT/PROXY |
| Back | In | No | 598 | 216 | — — | +3.37% [-4.75%, +11.64%] | +3.78% | in: use FLAT/PROXY |
...
**Escala Kelly usada nesta curva**: P99_PROXY | ref_back=4395.45 ref_lay=2041.46 | cap_back=2.0% cap_lay=1.0% | max_stake_event=100%*limit
...
| Filtro | Jogos únicos |
|---|---:|
| Combinações elegíveis (edge + timing + t0) | 258 |
| Com ROI disponível (precisa de placar) | 229 |
...
|---|---|---:|---:|---:|---:|---:|---:|
| 2026-02-11→2026-02-13 | 2026-02-14→2026-02-14 | 1 | 44 | +13.53% [-10.44%, +39.86%] | 715.83 | -52.58 | -52.58 |
| 2026-02-12→2026-02-14 | 2026-02-15→2026-02-15 | 3 | 56 | +15.77% [-3.07%, +34.20%] | 1126.61 | 241.65 | 256.66 |
...
| Dias OOS usados | 2 |
| Turnover 30d (proj.) | 27636.63 |
| Turnover 30d (Pre/In) | 40847.03 / 383765.81 |
| Lucro 30d (obs.) | 2836.08 |
| Lucro 30d (obs.) Pre/In | 2681.60 / -85825.03 |
| Lucro 30d (exp.) | 3061.15 |
| Lucro 30d (exp.) Pre/In | 2681.60 / -86734.45 |
| Banca risco p99 (Back+Lay) | 1122.51 |
| Banca liquidez p99 (+buf) | 741.13 |
| Banca recomendada (max) | 1122.51 |
| ROI/banca 30d (obs.) | 252.66% |
| ROI/banca 30d (exp.) | 272.71% |
| DD 30d p95 (obs.) | 368.08 |
| DD 30d p95 (exp.) | 420.66 |
...
Referência de banca p/ budget: 1122.51 | budgets por jogo aplicados em stake (Back) e liability (Lay).

| Cenário | Turnover 30d | Lucro 30d (exp.) | Banca rec. (max) | ROI/banca 30d (exp.) | DD 30d p95 (exp.) |
|---|---:|---:|---:|---:|---:|
| BASELINE (sem budget) | 27636.63 | 3061.15 | 1122.51 | 272.71% | 420.66 |
| BUDGET_0.50%/0.25% cap25% | 2983.75 | 303.05 | 116.12 | 260.98% | 19.40 |
| BUDGET_1.00%/0.50% cap33% | 7501.90 | 786.00 | 295.75 | 265.77% | 94.91 |
| BUDGET_2.00%/1.00% cap50% | 20753.76 | 2529.91 | 817.84 | 309.34% | 249.40 |
...
| Indicador | Valor |
|---|---:|
| Jogos únicos no recorte | 344 |
| Jogos com placar disponível (home_score/away_score não nulos) | 299 |
| Jogos com status='finished' no banco | 299 |
